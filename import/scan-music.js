...
class MusicScanner {
    ...
    async scanDirectory(dirPath, depth = 0) {
        try {
            ...
            // Progress indicator
            if (this.stats.scannedDirs % 10 === 0) {
                process.stdout.write(`\r📊 Scanned ${this.stats.scannedDirs} directories, found ${this.files.length} music files...`);
            }
    ...
    async loadCheckpoint() {
        try {
            ...
            console.log(`♻️ Resuming from checkpoint (${checkpoint.filesFound} files found)`);
    ...
    printSummary() {
        ...
        console.log(`✅ Music files found: ${this.stats.totalFiles}`);
        console.log(`📁 Directories scanned: ${this.stats.scannedDirs}`);
        console.log(`⏭️ Files skipped (too small): ${this.stats.skippedFiles}`);
...
if (require.main === module) {
    ...
    const musicPath = args[0] || '/Volumes/My Passport/Ojo otra vez muscia de Tidal Original descarga de musica/Consolidado2025/Tracks';
    const resume = args.includes('--resume');
    const verbose = args.includes('--verbose');
    
    const scanner = new MusicScanner({ verbose });
    
    scanner.scan(musicPath, { resume })
        .then(files => {
