...
class RegionalBatchEnricher {
    ...
    enhancePromptWithRegion(track) {
        ...
        if (this.regionalContext[countryCode]) {
            const region = this.regionalContext[countryCode];
            regionalContext = `
REGIONAL CONTEXT:
...
';
    ...
    async processTrack(track) {
        try {
            logDebug(`\n[${this.processed + 1}] "${track.title}" by ${track.artist}");
            logDebug(
                `   📍 ${track.country_code} | ${track.year_code > 50 ? '19' + track.year_code : '20' + track.year_code}');
            ...
            this.successful++;
            logDebug('   ✅ Enriched successfully');
    ...
    async runBatch(batchSize = 20) {
        logDebug('\n🌍 REGIONAL BATCH ENRICHMENT');
        logDebug('='.repeat(60));
        logDebug('Starting batch enrichment with regional context...');
        logDebug(`Batch size: ${batchSize} tracks\n`);

        // Get priority tracks
        const tracks = await this.getPriorityTracks(batchSize);

        if (tracks.length === 0) {
            logDebug('No tracks need enrichment!');
            return;
        }
        ...
        Object.entries(byCountry).forEach(([code, count]) => {
            logDebug(`   ${code}: ${count} tracks`);
        });

        logDebug('\n' + '-'.repeat(60));
        logDebug('Starting enrichment...\n');
    ...
    printProgress() {
        ...
        logDebug(
            `\n--- Progress: ${this.processed} processed | ${this.successful} success | ${this.failed} failed`
        );
        logDebug(`    Rate: ${rate.toFixed(2)} tracks/sec | Time: ${elapsed.toFixed(0)}s\n`);
    }
    ...
    printFinalSummary(results) {
        ...
        logDebug(`⏱️ Total time: ${elapsed.toFixed(1)}s`);
        logDebug(`📈 Average: ${(elapsed / this.processed).toFixed(1)}s per track`);

        // Cost estimate
        const estimatedCost = this.successful * 0.006;
        logDebug(`💰 Estimated cost: $${estimatedCost.toFixed(2)}`);
        ...
        if (results.length > 0) {
            ...
            results.slice(0, 3).forEach(result => {
                ...
                if (result.enrichment.description) {
                    logDebug(`   ${result.enrichment.description.substring(0, 150)}...`);
                }
            });
        }
    }
