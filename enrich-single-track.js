...
async function analyzeTrack(track) {
    logDebug('\n📝 Analizando track:');
    logDebug(`   Artista: ${track.artist}`);
    logDebug(`   Título: ${track.title}`);
    logDebug(`   Álbum: ${track.album}`);
    logDebug(`   Género actual: ${track.genre}`);
    logDebug(`   BPM: ${track.existing_bmp}`);
    logDebug(`   Key: ${track.existing_key}`);
    logDebug();

    const prompt = `Analyze this 80s pop/dance track and provide detailed metadata:
...
Provide a JSON response with:
{
    ...
    "occasions": ["party", "dance", "celebration"],
    "tempo_description": "fast, upbeat",
                ...
                messages: [
                    {
                        role: 'system',
                        content:
                            'You are a music analysis expert specializing in 1980s pop and dance music.'
                    },
...
async function updateDatabase(db, fileId, analysis) {
    return new Promise((resolve, reject) => {
        // Primero verificar si existe registro en llm_metadata
        db.get('SELECT * FROM llm_metadata WHERE file_id = ?', [fileId], (err, row) => {
            if (err) {
                reject(err);
                return;
            }
...
const filePath =
    '/Volumes/My Passport/Ojo otra vez muscia de Tidal Original descarga de musica/Consolidado2025/Tracks/Cyndi Lauper - Girls Just Want to Have Fun (12 Version).flac';

logDebug(`
🎵 ENRIQUECIMIENTO DE TRACK CON AI
${'='.repeat(60)}
');
