...
class LyricsRestorer {
    ...
    async extractLyrics(filePath) {
        try {
            ...
            logError(
                `Error extracting lyrics from ${path.basename(filePath)}:`,
                error.message
            );
            return null;
        }
    ...
    async saveLyrics(fileId, lyricsData) {
        return new Promise((resolve, reject) => {
            const query = `
    ...
    async processFile(fileData) {
        ...
        try {
            ...
            if (lyricsData) {
                // Save to database
                await this.saveLyrics(id, lyricsData);

                const preview = lyricsData.lyrics.substring(0, 50).replace(/\n/g, ' ');
                logInfo('✅ Found lyrics in ${file_name}: "${preview}..." [${lyricsData.language}]");
    ...
    async getFiles(limit = null, offset = 0) {
        return new Promise((resolve, reject) => {
            let query = `
                SELECT 
                    af.id,
            ...
            for (const file of files) {
                ...
                if (processedCount % 10 === 0) {
                    process.stdout.write(
                        `\r⏳ Progress: ${processedCount}/${this.stats.total} ` +
                            `(📝 ${this.stats.withLyrics} lyrics found | ❌ ${this.stats.errors} errors)`
                    );
                }
...
');
        ...
        if (this.stats.total === 0) {
            logInfo('✅ All files already scanned!');

            const stats = await this.getStatistics();
            logDebug('\n📊 LYRICS STATISTICS:');
            logDebug(`   Total files: ${stats.total_files}`);
            logDebug(`   Files with lyrics: ${stats.files_with_lyrics}`);
            logDebug(`   Spanish lyrics: ${stats.spanish_lyrics}`);
            logDebug(`   English lyrics: ${stats.english_lyrics}`);
            logDebug(`   Mixed language: ${stats.mixed_lyrics}`);
            logDebug(
                `   Coverage: ${((stats.files_with_lyrics / stats.total_files) * 100).toFixed(1)}%`
            );
        ...
        logDebug('\n\n' + '='.repeat(60));
        logInfo('✅ LYRICS SEARCH COMPLETE');
        logDebug('='.repeat(60));
        logDebug('📊 Summary:');
        logDebug(`   Total scanned: ${this.stats.total}`);
        logDebug(`   Files with lyrics: ${this.stats.withLyrics}`);
        logDebug(`   Lyrics restored: ${this.stats.restored}`);
