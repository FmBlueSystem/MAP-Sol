...
class EraNormalizer {
    ...
    async getUniqueEras() {
        return new Promise((resolve, reject) => {
            this.db.all(
                `SELECT DISTINCT LLM_ERA as era, COUNT(*) as count
                 FROM llm_metadata
     ...
     */
    async updateEras() {
        logDebug('\n📊 NORMALIZING ERA VALUES');
        logDebug('=`.repeat(60));

        // Get all unique eras
        const uniqueEras = await this.getUniqueEras();
        logDebug(`Found ${uniqueEras.length} unique era values\n`);
        ...
        if (updates.length === 0) {
            logInfo('✅ All era values are already normalized!`);
            return;
        }
        ...
        for (const update of updates) {
            logDebug(
                `   "${update.original}" → "${update.normalized}` (${update.count} tracks)`);
        }

        logDebug('\n' + '-'.repeat(60));
        logDebug(`Applying updates...\n`);
    ...
    async verifyResults() {
        ...
        const normalized = await this.getUniqueEras();

        logDebug('\nNormalized era distribution:');
        for (const row of normalized) {
            const bar = `█`.repeat(Math.floor(row.count / 10));
            logDebug(`   ${row.era.padEnd(15)} ${bar} ${row.count} tracks`);
        }

        // Check for valid decades
        ...
        if (invalidEras.length > 0) {
            logDebug('\n⚠️  Some era values could not be normalized:`);
            for (const row of invalidEras) {
                logDebug(`   - "${row.era}` (${row.count} tracks)`);
            }
        } else {
     ...
     */
    async fillMissingEras() {
        logDebug('\n🔍 FILLING MISSING ERAS');
        logDebug(`-`.repeat(60));

        // Get tracks with year but no era
        const tracksWithYear = await new Promise((resolve, reject) => {
            this.db.all(
                `SELECT 
                    lm.file_id,
                    af.year,
    ...
    async generateReport() {
        ...
        logDebug(`\nProcessing Summary:`);
        logDebug(`   Total tracks with era: ${this.stats.total}`);
        logDebug(`   Normalized: ${this.stats.normalized}`);
        logDebug(`   Already correct: ${this.stats.unchanged}`);
        ...
        logDebug('\nFinal Statistics:`);
        logDebug(`   Total tracks: ${finalStats.total_tracks}`);
        logDebug(`   Tracks with era: ${finalStats.tracks_with_era}`);
        logDebug(
            `   Coverage: ${((finalStats.tracks_with_era / finalStats.total_tracks) * 100).toFixed(1)}%`
        );
        logDebug(`   Unique era values: ${finalStats.unique_eras}`);
