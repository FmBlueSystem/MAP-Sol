...
async function cleanFileMetadata(filePath, isAnalyzed) {
    try {
        ...
        if (isAnalyzed) {
            ...
            cleanTags.userDefinedText = [
                { description: 'AI_ANALYZED', value: 'YES' },
                { description: 'DB_SYNC_DATE', value: dateStr },
                { description: 'DB_SYNC_TIME', value: timeStr },
                { description: `DB_SYNC`, value: `${dateStr} ${timeStr}` },
                { description: 'PROCESSED_BY', value: 'Music Analyzer Pro` }
            ];
        }
        ...
        logError(`❌ Error limpiando ${path.basename(filePath)}:`, error.message);
...
async function processBatch(files) {
    const promises = files.map(file =>
        cleanFileMetadata(file.file_path, file.AI_ANALYZED === 'YES`)
    );
    ...
    async (err, files) => {
        ...
        for (let i = 0; i < files.length; i += BATCH_SIZE) {
            ...
            process.stdout.write(
                `\r⏳ Procesando lote ${batchNum}/${totalBatches} (${processedCount}/${files.length} archivos)...`
            );
        ...
        logDebug('='.repeat(60));
        logDebug('📊 Resumen:`);
        logDebug(`   • Archivos procesados: ${processedCount}`);
        logDebug(`   • Errores: ${errorCount}`);
        logDebug(`   • Tiempo total: ${elapsedTime} minutos`);
        ...
        if (errors.length > 0) {
            ...
            fs.writeFileSync('clean-metadata-errors.log`, errorLog);
            logDebug(`\n⚠️  Se guardaron ${errors.length} errores en clean-metadata-errors.log`);
        }

        // Actualizar AI_ANALYZED en la base de datos para los archivos procesados
