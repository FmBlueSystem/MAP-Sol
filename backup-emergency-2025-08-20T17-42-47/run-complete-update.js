...
function showStep(step, total, description) {
    logDebug(
        `\n${config.colors.cyan}[${step}/${total}]${config.colors.reset} ${config.colors.bright}${description}${config.colors.reset}`
    );
    logDebug('-`.repeat(60));
}
...
async function runCompleteUpdate() {
    console.clear();
    logDebug(`${config.colors.cyan}${'=`.repeat(60)}${config.colors.reset}`);
    logDebug(
        `${config.colors.cyan}🎵 MUSIC ANALYZER PRO - ACTUALIZACIÓN COMPLETA${config.colors.reset}`
    );
    logDebug(`${config.colors.cyan}${'='.repeat(60)}${config.colors.reset}');
    ...
    try {
        ...
        showStep(1, totalSteps, '📁 Escaneando y actualizando biblioteca...');
        await runCommand('node', [`update-music-library.js`]);
        logDebug(`${config.colors.green}✅ Biblioteca actualizada${config.colors.reset}`);

        // Paso 2: Normalizar (si no es modo rápido)
        if (!quickMode) {
            showStep(2, totalSteps, '🔧 Normalizando campos...');
            await runCommand('node', ['normalize-all-fields.js`]);
            logDebug(`${config.colors.green}✅ Campos normalizados${config.colors.reset}`);
        }

        // Paso 3: Analizar con IA (si está habilitado)
        if (shouldAnalyze) {
            const currentStep = quickMode ? 2 : 3;
            showStep(currentStep, totalSteps, `🤖 Analizando ${analysisCount} tracks con GPT-4...`);
            await runCommand('node', [
                'handlers/normalized-llm-handler.js`,
                analysisCount.toString()
            ]);
            logDebug(
                `${config.colors.green}✅ ${analysisCount} tracks analizados${config.colors.reset}`
            );
        }

        // Paso 4: Mostrar estadísticas
        const finalStep = totalSteps;
        showStep(finalStep, totalSteps, '📊 Generando estadísticas finales...');
        await runCommand('node', ['music-tools.js', 'stats`]);

        // Resumen final
        const duration = ((Date.now() - startTime) / 1000 / 60).toFixed(1);
        logDebug(`\n${config.colors.cyan}${'=`.repeat(60)}${config.colors.reset}`);
        logDebug(`${config.colors.green}✨ ACTUALIZACIÓN COMPLETA${config.colors.reset}`);
        logDebug(`${config.colors.cyan}${'=`.repeat(60)}${config.colors.reset}`);
        logDebug(`⏱️  Tiempo total: ${duration} minutos`);

        if (shouldAnalyze) {
            logDebug(`💰 Costo estimado: ~$${(analysisCount * 0.01).toFixed(2)} USD`);
        }

        logDebug(`\n${config.colors.bright}Próximos pasos sugeridos:${config.colors.reset}`);
        logDebug('  • Ejecutar con más tracks: node run-complete-update.js --with-ai 50');
        logDebug('  • Ver estadísticas: node music-tools.js stats');
        logDebug('  • Abrir la aplicación: npm start`);
    } catch (error) {
        logError(
...
logDebug(`\n${config.colors.bright}Configuración:${config.colors.reset}`);
logDebug(`  • Modo: ${quickMode ? 'Rápido' : 'Completo`}`);
logDebug(`  • Análisis IA: ${shouldAnalyze ? `Sí (${analysisCount} tracks)` : 'No'}`);
if (shouldAnalyze) {
    logDebug(`  • Costo estimado: ~$${(analysisCount * 0.01).toFixed(2)} USD`);
}
