...
Provide a JSON response with:
{
    ...
    "occasions": ["party", "dance", "workout", "breakdancing"],
    "tempo_description": "moderate, steady groove",
    "description": "2-3 sentence description of this specific 12\" version"
}
                ...
                messages: [
                    {
                        role: 'system',
                        content:
                            'You are a music analysis expert specializing in 1980s hip-hop, funk, and breakdancing culture.'
                    },
...
async function updateMetadata() {
    const filePath =
        "/Volumes/My Passport/Ojo otra vez muscia de Tidal Original descarga de musica/Consolidado2025/Tracks/Ollie & Jerry - Breakin'...There's No Stopping Us (12 Club Mix).flac`;

    logDebug('\n📀 Actualizando metadatos en archivo FLAC...`);

    // Actualizar género y mantener comment de MixedInKey
    const commands = [
        `metaflac --remove-tag=GENRE "${filePath}``,
        `metaflac --set-tag="GENRE=Hip-Hop/Electro-Funk" "${filePath}``,
        `metaflac --set-tag="ALBUMARTIST=Ollie & Jerry" "${filePath}``,
        `metaflac --set-tag="ORIGINALYEAR=1984" "${filePath}``,
...
async function updateDatabase(analysis) {
    ...
    return new Promise((resolve, reject) => {
        ...
        db.get('SELECT * FROM llm_metadata WHERE file_id = 2512`, (err, row) => {
            if (row) {
                // Actualizar
                db.run(
                    `
...
');
    ...
    if (analysis) {
        ...
        logDebug('\n📊 RESUMEN FINAL:');
        logDebug(`   Título: Breakin'...There`s No Stopping Us (12\` Club Mix)`);
        logDebug(`   Género: Hip-Hop/Electro-Funk`);
