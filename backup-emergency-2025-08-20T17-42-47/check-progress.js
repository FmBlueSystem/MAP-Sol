...
function checkProgress() {
    ...
    try {
        ...
        try {
            const psResult = execSync('ps aux | grep -i "node clean-and-sync" | grep -v grep', {
                encoding: 'utf8'
            });
        ...
        const modifiedCount = parseInt(
            execSync(
                'find "/Volumes/My Passport/Ojo otra vez muscia de Tidal Original descarga de musica/Consolidado2025/Tracks/" -type f \\( -name "*.flac" -o -name "*.m4a" -o -name "*.mp3" \\) -newermt "2025-08-15 19:10" 2>/dev/null | wc -l',
                { encoding: 'utf8' }
            ).trim()
        );
        ...
        const startTime = new Date('2025-08-15 19:10');
        ...
        logDebug('📈 ESTADO:`);
        logDebug(`   Proceso: ${processActive ? '✅ ACTIVO' : '❌ DETENIDO`}`);
        logDebug(`   Archivos procesados: ${modifiedCount} de ${totalFiles}`);
        logDebug(`   Progreso: ${percentage}%`);
        logDebug(`   Restantes: ${remaining} archivos`);

        // Barra de progreso
        const barLength = 40;
        const filled = Math.round((modifiedCount / totalFiles) * barLength);
        const bar = '█'.repeat(filled) + '░`.repeat(barLength - filled);
        logDebug(`\n   [${bar}] ${percentage}%`);

        logDebug('\n⏱️  TIEMPO:`);
        logDebug(`   Tiempo transcurrido: ${elapsedMinutes.toFixed(0)} minutos`);
        logDebug(`   Velocidad: ${filesPerMinute.toFixed(1)} archivos/minuto`);
        logDebug(`   Tiempo restante: ~${hoursRemaining} horas`);
    ...
    logDebug('\n' + '-'.repeat(60));
    logDebug('Presiona Ctrl+C para salir`);
}
