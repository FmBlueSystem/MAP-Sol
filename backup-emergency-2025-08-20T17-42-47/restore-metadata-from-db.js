...
reject(err);
    ...
    async checkFileMetadata(filePath) {
        try {
            ...
            if (ext === '.mp3') {
                ...
                const hasTitle = tags.title || tags.Title || tags['©nam'];
                const hasArtist = tags.artist || tags.Artist || tags['©ART'];
                const hasAlbum = tags.album || tags.Album || tags['©alb'];
                ...
                try {
                    const { stdout } = await execPromise(
                        `metaflac --list --block-type=VORBIS_COMMENT "${filePath}``);
                    ...
                    return {
                        hasMetadata: !!(hasTitle || hasArtist || hasAlbum),
                        title: hasTitle ? 'Has title' : undefined,
                        artist: hasArtist ? 'Has artist' : undefined,
                        album: hasAlbum ? 'Has album' : undefined
                    };
                } catch {
    ...
    async restoreMetadataToFile(fileData) {
        ...
        try {
            ...
            const currentMeta = await this.checkFileMetadata(file_path);
            if (currentMeta.hasMetadata) {
                logDebug(`⏭️  Ya tiene metadatos: ${path.basename(file_path)}`);
                this.stats.skipped++;
                return false;
            }
            ...
            if (ext === '.mp3') {
                ...
                // Restaurar en M4A/MP4 con ffmpeg
                const tempFile = file_path + `.temp.m4a`;

                let cmd = `ffmpeg -i "${file_path}` -codec copy`;
                if (title) {
...
cmd += ` -metadata album="${album.replace(/"/g, '\\``)}``;
                ...
                if (genre) {
cmd += ` -metadata genre="${genre.replace(/"/g, '\\``)}``;
}
                if (year) {
cmd += ` -metadata year="${year}``;
}
                if (existing_bmp) {
cmd += ` -metadata BPM="${Math.round(existing_bmp)}``;
}
                if (isrc) {
cmd += ` -metadata ISRC="${isrc}``;
}
                ...
                logInfo('✅ Restaurado M4A: ${artist} - ${title}');
                this.stats.restored++;
                return true;
            } else if (ext === `.flac`) {
                ...
                const commands = [];
                if (title) {
commands.push(`metaflac --set-tag="TITLE=${title}" "${file_path}``);
}
                if (artist) {
commands.push(`metaflac --set-tag="ARTIST=${artist}" "${file_path}``);
}
                if (album) {
commands.push(`metaflac --set-tag="ALBUM=${album}" "${file_path}``);
}
                if (genre) {
commands.push(`metaflac --set-tag="GENRE=${genre}" "${file_path}``);
}
                if (year) {
commands.push(`metaflac --set-tag="DATE=${year}" "${file_path}``);
}
                if (existing_bmp) {
                {
commands.push(
                    `metaflac --set-tag="BPM=${Math.round(existing_bmp)}" "${file_path}``);
                }
                if (isrc) {
commands.push(`metaflac --set-tag="ISRC=${isrc}" "${file_path}``);
}
            ...
            logError(`❌ Error restaurando ${path.basename(file_path)}: ${error.message}`);
            this.stats.errors++;
...
resolve(rows);
    ...
    async run(options = {}) {
        const { limit = null, checkOnly = false } = options;
...
');
        ...
        logDebug('📊 Archivos a verificar: ${this.stats.total}');
        ...
        if (checkOnly) {
            ...
            for (let i = 0; i < files.length; i++) {
                const file = files[i];
                process.stdout.write(`\r⏳ Verificando: ${i + 1}/${this.stats.total}`);
            ...
            logDebug(`   Total verificados: ${this.stats.total}`);
            logDebug(`   Sin metadatos: ${noMetadataCount}`);
            logDebug(`   Con metadatos: ${this.stats.total - noMetadataCount}`);

            if (affectedFiles.length > 0) {
                logDebug('\n❌ ARCHIVOS SIN METADATOS (primeros 10):`);
                affectedFiles.slice(0, 10).forEach(f => {
                    logDebug(
                        `   • ${f.artist || 'Unknown'} - ${f.title || path.basename(f.path)}');
                });

                logDebug('\n💡 Para restaurar, ejecuta:');
                logDebug('   node restore-metadata-from-db.js --restore');
            } else {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                process.stdout.write(
                    `\r⏳ Procesando [${this.stats.processed}/${this.stats.total}] Restaurados: ${this.stats.restored} | Omitidos: ${this.stats.skipped} | Errores: ${this.stats.errors}`
                );
            ...
            logDebug(`   Omitidos (ya tenían): ${this.stats.skipped}`);
...
if (require.main === module) {
    ...
    for (let i = 0; i < args.length; i++) {
        if (args[i] === '--limit' && args[i + 1]) {
            ...
            logDebug(`
Uso: node restore-metadata-from-db.js [opciones]
