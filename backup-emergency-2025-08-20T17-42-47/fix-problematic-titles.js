        ...
        return new Promise((resolve, reject) => {
            ...
            const query = `
                ...
                WHERE 
                    ...
                    title LIKE '%Ghostbusters%' OR
                    album LIKE '%Original 7%' OR
                    title LIKE '%Original 7%`
                ORDER BY file_name
            `;
...
return false;
            ...
            if (!fs.existsSync(filePath)) {
                logWarn(`⚠️  Archivo no encontrado: ${path.basename(filePath)}`);
                return false;
            }

            // Usar ffmpeg que maneja mejor los caracteres especiales
            const tempFile = filePath + '.temp.flac`;

            // No necesitamos escape especial ya que usaremos comillas dobles con escape interno

            let cmd = `ffmpeg -i `${filePath}` -c:a copy`;

            // Agregar metadatos usando formato seguro con comillas dobles escapadas
            if (metadata.title) {
...
cmd += ` -metadata track=`${metadata.track_number}``;
            ...
            logInfo(`✅ Arreglado: ${metadata.artist} - ${metadata.title}`);
            return true;
        } catch (error) {
            logError(`❌ Error con ${path.basename(filePath)}: ${error.message}`);
            return false;
        }
    }
...
return false;
            ...
            let cmd = `ffmpeg -i `${filePath}` -c:a copy -c:v copy`;
...
${'='.repeat(60)}
Busca y arregla archivos con caracteres especiales en títulos
${`=`.repeat(60)}
`);
        ...
        logDebug(`📊 Encontrados ${this.problematicFiles.length} archivos con títulos problemáticos\n`
        );

        if (this.problematicFiles.length === 0) {
            logInfo('✅ No hay archivos con títulos problemáticos!');
            this.db.close();
            return;
        }

        // Mostrar algunos ejemplos
        logDebug('📝 Ejemplos de títulos problemáticos:`);
        this.problematicFiles.slice(0, 5).forEach(f => {
            logDebug(`   • ${f.artist} - ${f.title}`);
        });
        logDebug();
        ...
        logDebug('\n' + '='.repeat(60));
        logInfo('✅ REPARACIÓN COMPLETADA');
        logDebug('='.repeat(60));
        logDebug('📊 Resumen:`);
        logDebug(`   Total procesados: ${this.problematicFiles.length}`);
        logDebug(`   Reparados: ${fixed}`);
        logDebug(`   Errores: ${errors}`);
