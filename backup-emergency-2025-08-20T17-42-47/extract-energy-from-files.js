...
async function extractMetadata(filePath) {
    try {
        ...
        logError(`Error extracting metadata from ${filePath}:`, error.message);
        return { energyLevel: null, comment: null, aiEnergy: null };
    }
}
...
async function updateEnergyLevels() {
    logInfo('🚀 Starting energy level extraction...`);
    ...
    for (const file of files) {
        ...
        if (processed % 10 === 0) {
            logDebug(`Progress: ${processed}/${files.length} (${updated} updated)`);
        }
                    ...
                    err => {
                        if (err) {
                            ...
                            if (metadata.energyLevel) {
                                logInfo(`✅ Updated ${path.basename(file.file_path)}: Energy ${metadata.energyLevel}`
                                );
                                updated++;
                            }
                            resolve();
    ...
    `,
        (err, row) => {
            if (!err && row) {
                logDebug('\n📊 Final Statistics:`);
                logDebug(`Total files: ${row.total}`);
                logDebug(`With energy_level: ${row.with_energy}`);
                logDebug(`With comment: ${row.with_comment}`);
                logDebug(`With AI_ENERGY: ${row.with_ai_energy}`);
            }
