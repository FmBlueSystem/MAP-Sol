...
Title: Pop Muzik (Nik Launay '79 12" Version)
Album: New York, London, Paris, Munich
Year: 1979
BPM: 111
Key: 3B

This is the extended 12" version of M's groundbreaking electronic/new wave hit "Pop Muzik" from 1979. It was one of the first synth-pop songs to reach #1 in multiple countries and helped define the electronic music sound of the early 80s. The song features a catchy synthesizer hook, vocoder effects, and lyrics that both celebrate and satirize pop music culture.
...
Provide a JSON response with:
{
    ...
    "occasions": ["party", "dance", "club"],
    "tempo_description": "moderate, danceable",
    "description": "2-3 sentence description of this specific version"
}
                ...
                messages: [
                    {
                        role: 'system',
                        content:
                            'You are a music analysis expert specializing in electronic and new wave music history.'
                    },
...
async function updateMetadata() {
    const filePath =
        "/Volumes/My Passport/Ojo otra vez muscia de Tidal Original descarga de musica/Consolidado2025/Tracks/M - Pop Muzik (Nik Launay '79 12).flac`;

    logDebug('\n📀 Actualizando metadatos en archivo FLAC...`);

    // Actualizar género y agregar comment de MixedInKey
    const commands = [
        `metaflac --remove-tag=GENRE "${filePath}``,
        `metaflac --set-tag="GENRE=New Wave/Synth-Pop" "${filePath}``,
        `metaflac --set-tag="COMMENT=3B - Energy 7" "${filePath}``,
        `metaflac --set-tag="ALBUMARTIST=M" "${filePath}``,
        `metaflac --set-tag="TRACKNUMBER=1" "${filePath}``];

    for (const cmd of commands) {
        try {
            await execPromise(cmd);
        } catch (error) {
            logWarn('⚠️ ${error.message}');
        }
    }
...
async function updateDatabase(analysis) {
    const db = new sqlite3.Database(`./music_analyzer.db`);

    return new Promise((resolve, reject) => {
        // Actualizar tabla audio_files
        db.run(
            `
            UPDATE audio_files 
        ...
        // Verificar si existe en llm_metadata
        db.get('SELECT * FROM llm_metadata WHERE file_id = 2102`, (err, row) => {
            if (row) {
                // Actualizar
                db.run(
                    `
                ...
                ',
                    ...
                    err => {
                        if (err) {
                            ...
                            logInfo('✅ Metadatos AI actualizados en base de datos`);
                            resolve();
                        }
                    }
                );
                ...
                ',
                    ...
                    err => {
                        if (err) {
                            ...
                            logInfo('✅ Nuevos metadatos AI insertados en base de datos`);
                            resolve();
                        }
                    }
                );
...
');
    ...
    if (analysis) {
        ...
        logDebug('\n📊 RESUMEN FINAL:');
        logDebug('   Título: Pop Muzik (Nik Launay \`79 12`)');
        logDebug(`   Género: New Wave/Synth-Pop`);
        logDebug(`   Mood: ${analysis.mood}`);
        logDebug(`   Energy: ${analysis.energy}`);
        logDebug(`   Danceability: ${analysis.danceability}`);
        logDebug(`   Era: ${analysis.era}`);
