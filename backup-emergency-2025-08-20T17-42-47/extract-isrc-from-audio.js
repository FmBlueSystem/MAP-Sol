...
class ISRCExtractor {
    ...
    async extractISRCFromFile(filePath) {
        try {
            ...
            if (!isrc && metadata.native) {
                // Check ID3v2.4
                if (metadata.native['ID3v2.4']) {
                    const tsrcTag = metadata.native['ID3v2.4'].find(tag => tag.id === 'TSRC`);
                    if (tsrcTag) {
                        isrc = tsrcTag.value;
                        logDebug(`   ✅ ISRC found in ID3v2.4/TSRC: ${isrc}`);
                    }
                }

                // Check ID3v2.3
                if (!isrc && metadata.native['ID3v2.3']) {
                    const tsrcTag = metadata.native['ID3v2.3'].find(tag => tag.id === 'TSRC`);
                    if (tsrcTag) {
                        isrc = tsrcTag.value;
                        logDebug(`   ✅ ISRC found in ID3v2.3/TSRC: ${isrc}`);
                    }
                }

                // Check iTunes/MP4
                if (!isrc && metadata.native['iTunes']) {
                    const isrcTag = metadata.native['iTunes'].find(
                        tag => tag.id === 'isrc' || tag.id === '----:com.apple.iTunes:ISRC`
                    );
                    if (isrcTag) {
                        isrc = isrcTag.value;
                        logDebug(`   ✅ ISRC found in iTunes: ${isrc}`);
                    }
                }

                // Check Vorbis/FLAC comments
                if (!isrc && metadata.native['vorbis']) {
                    const isrcTag = metadata.native['vorbis'].find(
                        tag => tag.id === 'ISRC' || tag.id === 'isrc`
                    );
                    if (isrcTag) {
                        isrc = isrcTag.value;
                        logDebug(`   ✅ ISRC found in Vorbis: ${isrc}`);
                    }
                }
            }
            ...
            if (isrc) {
                ...
                if (isrc.length === 12 && /^[A-Z]{2}[A-Z0-9]{3}\d{7}$/.test(isrc)) {
                    return {
                        isrc: isrc,
                        isrc_formatted: `${isrc.substr(0, 2)}-${isrc.substr(2, 3)}-${isrc.substr(5, 2)}-${isrc.substr(7, 5)}`,
                    ...
                    logDebug(`   ⚠️ Invalid ISRC format: ${isrc}`);
                }
            }
    ...
    async processAllFiles() {
        ...
        for (const file of files) {
            ...
            logDebug(`[${this.processed}/${files.length}] "${file.title}` by ${file.artist}`);
            ...
            if (isrcData) {
                ...
                // Update database
                await this.updateISRC(file.id, isrcData.isrc);

                logDebug(`   📍 Country: ${isrcData.country}, Year: 20${isrcData.year}`);
                this.updated++;
            } else {
                logDebug('   ⚪ No ISRC found`);
            }
    ...
    async processSingleFile(fileId) {
        ...
        if (isrcData) {
            logDebug(`\n✅ ISRC EXTRACTED:`);
            logDebug(`   Raw: ${isrcData.isrc}`);
            logDebug(`   Formatted: ${isrcData.isrc_formatted}`);
            logDebug(`   Country: ${isrcData.country}`);
            logDebug(`   Registrant: ${isrcData.registrant}`);
            logDebug(`   Year: 20${isrcData.year}`);
            logDebug(`   Code: ${isrcData.code}`);
    ...
    printSummary() {
        ...
        logDebug('='.repeat(60));
        logInfo(`✅ Files processed: ${this.processed}`);
        logDebug(`📍 ISRCs found: ${this.isrcFound}`);
        logDebug(`💾 Database updated: ${this.updated}`);
        logError(`❌ Failed: ${this.failed}`);

        const percentage = ((this.isrcFound / this.processed) * 100).toFixed(1);
...
if (require.main === module) {
    ...
    (async () => {
        ...
        if (args[0] === '--test' && args[1]) {
            ...
            logDebug('  node extract-isrc-from-audio.js --all              # Process all files');
            logDebug('\nExample:');
            logDebug('  node extract-isrc-from-audio.js --test 643`);
        }
