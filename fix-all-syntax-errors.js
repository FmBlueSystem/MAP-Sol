...
function fixFile(filePath) {
    ...
    content = content.replace(/(\w+)\s+&&\s+(\w+);\s*\?\s*([^:]+)\s*:\s*([^;]+);/gm, 
        (match, p1, p2, p3, p4) => {
            return `${p1} && ${p2} ? ${p3} : ${p4};`;
        });
    ...
    content = content.replace(/const analyzerBinary = bpm && decodedKey;\s*\?\s*path\.join/gm,
        'const analyzerBinary = bpm && decodedKey\n                    ? path.join');
    
    // Fix command template literals with incorrect quotes
    content = content.replace(/command\s*=\s*``([^`]+)`\s*`([^`]+)`([^`]*)`/g,
        'command = `$1 $2$3`');
