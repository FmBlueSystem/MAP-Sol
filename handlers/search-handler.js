    ...
    return async (event, searchTerm, filters = {}) => {
        try {
            ...
            const sanitizedFilters = {
                genre:
                    filters.genre && typeof filters.genre === 'string'
                        ? filters.genre.slice(0, 50)
                        : null,
                mood:
                    filters.mood && typeof filters.mood === 'string'
                        ? filters.mood.slice(0, 50)
                        : null,
                minBPM:
                ...
                maxEnergy:
                    filters.maxEnergy && !isNaN(filters.maxEnergy)
                        ...
                        : null,
                limit:
                    filters.limit && !isNaN(filters.limit)
                        ? Math.max(1, Math.min(1000, Number(filters.limit)))
                        : 500
            };
