...
function createEnergyFlowHandlers(db) {
            ...
            if (playlistId) {
                ...
                sql = `
                    SELECT 
                        ...
                        COALESCE(lm.AI_BPM, af.existing_bpm, 120) as bpm,
    ...
    const optimizeFlowHandler = async (event, tracks) => {
        return new Promise(resolve => {
            ...
            const sortedByEnergy = [...optimized].sort((a, b) => (a.energy || 0.5) - (b.energy || 0.5));

            // Create wave pattern
            const wavePattern = [];
            let lowIndex = 0;
            const highIndex = sortedByEnergy.length - 1;
