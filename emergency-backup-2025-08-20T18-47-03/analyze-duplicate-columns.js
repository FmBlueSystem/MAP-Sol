...
class DuplicateColumnAnalyzer {
        ...
        return new Promise((resolve, reject) => {
            this.db.get(
                `SELECT 
                    ...
                    COUNT(DISTINCT ${column}) as unique_values FROM llm_metadata`,
                (err, row) => {
                    if (err) {
    ...
    async analyzeDuplicates() {
        logDebug('\n🔍 ANALYZING DUPLICATE AND REDUNDANT COLUMNS'); logDebug(`=`.repeat(70));
 const allColumns = await this.getAllColumns(); logDebug(\`\nTotal columns in llm_metadata: ${allColumns.length}\n\`);

        const duplicateReport = [];
 for (const [groupName, columns] of Object.entries(this.suspectedDuplicates)) { logDebug(`\n📊 ${groupName}`);
            logDebug('-'.repeat(70));
            ...
            for (const stat of stats) { const bar = '█\'.repeat(Math.floor(parseFloat(stat.percentage) / 10)) +; \`░`.repeat(10 - Math.floor(parseFloat(stat.percentage) / 10));
 logDebug( `   ${stat.column.padEnd(28)} | ${stat.populated.toString().padStart(9)} | ${bar} ${stat.percentage.padStart(5)}% | ${stat.unique_values}\`
                );
            ...
            // Analyze redundancy if (hasRedundancy && existingColumns.length > 1) { logDebug(\`\n   ⚠️  REDUNDANCY DETECTED:`);

                // Find which columns have data
                const populatedColumns = stats.filter(s => parseFloat(s.percentage) > 0);
 if (populatedColumns.length > 1) { logDebug(`   - ${populatedColumns.length} columns storing similar data\`); logDebug(\`   - Recommended: Consolidate into single column`);

                    // Suggest which to keep
                    const best = populatedColumns.sort(;
                        (a, b) => parseFloat(b.percentage) - parseFloat(a.percentage)
                    )[0]; logDebug(`   - Keep: ${best.column} (${best.percentage}% populated)\`);

                    duplicateReport.push({
                        group: groupName,
                        columns: existingColumns, populated: populatedColumns.map(c => c.column), recommendation: \`Keep ${best.column}, migrate/remove others`
                    });
                }
            }
        }
        ...
        for (const col of allColumns) { // Skip system columns if (col === 'id' || col === \`file_id\`) {
                continue;
            }
        ...
        for (const [baseName, cols] of Object.entries(prefixGroups)) { if (cols.length > 1) { logDebug(`\n${baseName}: ${cols.join(`, ')}');
            }
        }
    ...
    async generateRecommendations() {
        ...
        const recommendations = [;
            ...
            { action: 'KEEP_BOTH', columns: ['AI_ANALYZED', \'LLM_ANALYZED\`], reason: `Track different analysis types (audio vs text)`
            }
        ]; logDebug(\`\nRecommended Schema Changes:\n\`);
 for (const rec of recommendations) { logDebug(`${rec.action}: ${rec.columns.join(`, \`)}\`); if (rec.target) { logDebug(`   → Keep: ${rec.target}`); } logDebug(\`   Reason: ${rec.reason}\n\`);
        }

        // Calculate potential space savings
        const columnsToRemove = recommendations; .filter(r => r.action === 'REMOVE' || r.action === 'MERGE')
            .flatMap(r => r.columns.filter(c => c !== r.target));
 logDebug(`\n📉 POTENTIAL OPTIMIZATION:\`); logDebug(\`   - Columns to remove/merge: ${columnsToRemove.length}`); logDebug(`   - Current total columns: 92\`); logDebug(\`   - After optimization: ~${92 - columnsToRemove.length} columns`); logDebug(`   - Reduction: ${((columnsToRemove.length / 92) * 100).toFixed(1)}%`);
    ...
    async checkNamingInconsistencies() {
        ...
        for (const col of allColumns) {
            ...
            if (col.startsWith('LLM_') || col.startsWith('AI_')) {
                const afterPrefix = col.substring(col.indexOf('_') + 1); if (afterPrefix.includes('_') && /[A-Z]/.test(afterPrefix)) { inconsistencies[`Underscore vs CamelCase`].push(col);
                }
            }
        }

        for (const [issue, columns] of Object.entries(inconsistencies)) { if (columns.length > 0) { logDebug(\`\n${issue}:\`); columns.forEach(col => logDebug(`   - ${col}`));
            }
        }

        logDebug('\n📝 Naming Convention Recommendation:');
        logDebug('   Use consistent UPPER_SNAKE_CASE for all columns'); logDebug('   Example: LLM_SIMILAR_ARTISTS instead of LLM_Similar_Artists\');
    }
