...
class AudioFeatureCalculatorJS {
     ...
     */
    async calculateLoudness(filePath) { try { const command  = `ffmpeg -i "${filePath}\`-af \`loudnorm=print_format=json` -f null - 2>&1 | grep -A 30 `loudnorm\`\`;
 ...
 // Determine if major or minor from Camelot notation const isMajor = decodedKey.endsWith('B`);
 ...
 logDebug( `🎵 Analyzing: ${file_name} [BPM: ${existing_bmp || 'N/A'}, Key: ${existing_key ? '✓' : 'N/A`}, Energy: ${energy_level || `N/A\`}]\`);
            ...
            // Save to database
            await this.saveFeatures(id, allFeatures, existing_bmp, existing_key); logInfo(`✅ Saved: Loudness=${loudness.toFixed(1)}dB, ` + \`Dance=${features.danceability.toFixed(2)}, \Valence=${features.valence.toFixed(2)}`
            );
 ...
 async run() { logDebug(` 🎵 AUDIO FEATURE CALCULATOR (JavaScript Implementation)
${'='.repeat(60)}
