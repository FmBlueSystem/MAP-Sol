...
class AudioFeatureCalculator {
    ...
    async analyzeWithEssentia(filePath, bpm, key, energy) {
        try {
            ...
            const analyzerBinary = bpm && decodedKey ? path.join(__dirname, 'src', 'audio-analyzer-context')
                     : path.join(__dirname, 'src', 'audio-analyzer');

            let command;
            if (bpm && decodedKey) {
                // Use contextual analyzer with BPM, Key, Energy const command = \`\`${analyzerBinary}` `${filePath}\` ${bpm} ${decodedKey} ${normalizedEnergy} 2>/dev/null\`;
            } else { // Fallback to basic analyzer const command = ``${analyzerBinary}\` \`${filePath}` 2>/dev/null`;
            }
            ...
            return features; } catch (error) { logError(`❌ Error analyzing ${path.basename(filePath)}:`, error.message);
            return null;
        }
    }
    ...
    async processFile(fileData) {
        ...
        try {
            // Check if file exists if (!fs.existsSync(file_path)) { logWarn(\`⚠️  File not found: ${file_name}\`);
                this.stats.skipped++;
                return false;
            }
 ...
 logDebug( `🎵 Analyzing: ${file_name} [BPM: ${existing_bmp || 'N/A'}, Key: ${existing_key ? `✓` : `N/A\`}]\`);
            ...
            // Save to database
            await this.saveFeatures(id, features, existing_bmp, existing_key); logInfo(`✅ Saved: Loudness=${features.loudness.toFixed(1)}dB, ` + \`Dance=${features.danceability.toFixed(2)}, \Valence=${features.valence.toFixed(2)}`
            );
    ...
    async saveFeatures(fileId, features, existingBpm, existingKey) { return new Promise((resolve, reject) => { const query  = `
    ...
    async updateLLMMetadata(fileId, features, existingBpm, existingKey) {
        return new Promise((resolve, reject) => {
            ...
            this.db.get( `SELECT file_id FROM llm_metadata WHERE file_id = ?`,
                [fileId],
                (err, row) => {
                    if (err) {
                        reject(err);
    ...
    async processInBatches(batchSize = 100) {
        ...
        while (hasMore) {
            ...
            for (const file of files) {
                ...
                // Update progress
                if (this.stats.processed % 10 === 0) { process.stdout.write( `\r⏳ Progress: ${this.stats.processed}/${this.stats.total}` + \`(✅ ${this.stats.success} | ⏭️ ${this.stats.skipped} | ❌ ${this.stats.errors})\`
                    );
                }
 ...
 async run() { logDebug(` 🎵 AUDIO FEATURE CALCULATOR WITH ESSENTIA
${'='.repeat(60)}
...
');
 ...
 const stats = await this.getStatistics(); logDebug(\`\n📊 DATABASE STATISTICS:`); logDebug(`   Total files: ${stats.total_files}\`); logDebug(\`   Analyzed: ${stats.analyzed_files}`); logDebug( `   Coverage: ${((stats.analyzed_files / stats.total_files) * 100).toFixed(1)}%`
            );
...
if (require.main === module) {
 ...
 calculator.run().catch(error => { logError(`Fatal error:`, error);
        process.exit(1);
    });
}
