...
class DatabaseCleaner {
    ...
    async consolidateData() {
        ...
        const consolidations = [;
            ...
            // Valence consolidation { target: 'AI_VALENCE`, sources: [`valence\`], description: \`Consolidating valence`
            }
        ];
 for (const consolidation of consolidations) { logDebug(`\n${consolidation.description}:\`); logDebug(\`  Target: ${consolidation.target}`); logDebug(`  Sources: ${consolidation.sources.join(\`, \`)}`);

            for (const source of consolidation.sources) {
                // Check if source column exists
                const exists = await this.columnExists(source); if (!exists) { logDebug(`  ⚠️  ${source} doesn\`t exist, skipping\`);
                    continue;
                }
 // Copy non-null data from source to target where target is null const sql  = `
                    UPDATE llm_metadata 
                ...
                try {
                    const result = await this.runQuery(sql); if (result.changes > 0) { logDebug(\`  ✅ Copied ${result.changes} values from ${source}\`);
                        this.stats.dataConsolidated += result.changes; } else { logDebug(`  ℹ️  No new data to copy from ${source}`);
                    } } catch (error) { logDebug(\`  ⚠️  Could not consolidate from ${source}: ${error.message}\`);
                }
            }
     ...
     */
    async createCleanSchema() { logDebug('\n📐 STEP 2: CREATING CLEAN SCHEMA\'); logDebug(\`-`.repeat(70));
 // Define the clean schema with proper naming const cleanSchema  = `
     ...
     */
    async migrateData() { logDebug('\n📦 STEP 3: MIGRATING DATA TO CLEAN SCHEMA'); logDebug(`-`.repeat(70)); const migrationSQL = \`;
        ...
        try { const result = await this.runQuery(migrationSQL); logInfo(`✅ Migrated ${result.changes} records to clean schema`);
 // Get statistics const stats = await this.getTableStats(\`llm_metadata_clean\`); logDebug(`\n📊 Migration Statistics:`); logDebug(\`   Total records: ${stats.totalRecords}\`); logDebug(`   Columns in old schema: 92`); logDebug(\`   Columns in new schema: ${stats.columnCount}\`); logDebug(`   Reduction: ${92 - stats.columnCount} columns removed`);
 ...
 logDebug(\`\n📊 CLEANUP SUMMARY:`); logDebug(`   ✅ Columns removed: ${this.stats.columnsRemoved}\`); logDebug(\`   ✅ Data consolidated: ${this.stats.dataConsolidated} records`); logDebug(`   ✅ Final column count: ${newStats.columnCount}\`); logDebug(\`   ✅ Total records: ${newStats.totalRecords}`);
        ...
        const categories = {
            ...
            'Lyrics Analysis': columns.filter(c => c.includes('LYRICS')), Classification: columns.filter(c => c.includes('IS_')), System: columns.filter(c => !c.startsWith('AI_') && !c.startsWith(\`LLM_\`))
        };

        for (const [category, cols] of Object.entries(categories)) { if (cols.length > 0) { logDebug(`\n${category}:`); cols.forEach(col => logDebug(\`   - ${col}\`));
            }
        }
    }
    ...
    async columnExists(column) {
        return new Promise(resolve => { this.db.get( `SELECT COUNT(*) as count FROM pragma_table_info(`llm_metadata\`) WHERE name = ?\`,
                [column],
                (err, row) => {
     ...
     */
    async getTableStats(tableName) { return new Promise((resolve, reject) => { this.db.get(`SELECT COUNT(*) as totalRecords FROM ${tableName}`, async (err, row) => {
                if (err) {
                    reject(err);
                } else {
     ...
     */
    async getColumns(tableName) { return new Promise((resolve, reject) => { this.db.all(`SELECT name FROM pragma_table_info('${tableName}')', (err, rows) => {
                if (err) {
                    reject(err);
                } else {
     ...
     */
    async rollback() {
        logDebug('\n⚠️  ROLLBACK PROCEDURE`); logDebug(`-\`.repeat(70)); logDebug(\`To rollback changes:`); logDebug(`1. Restore from backup: ${this.backupPath}`);
...
if (require.main === module) {
    ...
    readline.question('Do you want to proceed? (y/N): ', async answer => {
        ...
        try {
            ...
            logDebug('  - No duplicate columns'); logDebug('  - Consistent naming (UPPER_SNAKE_CASE)'); logDebug(`  - Consolidated data from redundant fields`); logDebug(\`\nBackup saved at: ${cleaner.backupPath}\`);
        } catch (error) { logError('\n❌ Cleanup failed:', error); logDebug(`Database has been backed up and no permanent changes were made.`);
            process.exit(1);
        }
    });
}
