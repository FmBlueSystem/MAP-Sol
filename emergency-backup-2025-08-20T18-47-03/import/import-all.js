...
class MusicImporter {
    ...
    async run() {
        ...
        console.log(`📁 Source: ${this.musicPath}\`); console.log(\`🔧 Workers: ${this.workers}`); console.log(`📊 Limit: ${this.limit || 'None'}');
        console.log('=' .repeat(50) + '\n');
        ...
        try {
            ...
            // 3. Scan files
            let files = [];
            if (!this.checkpoint || !this.checkpoint.scanComplete) { console.log(\'\n📂 PHASE 1: Scanning files...\n\`);
                ...
                // Load from previous scan const scanResults = await fs.readFile(`scan-results.json`, \`utf8\`); files = JSON.parse(scanResults).files; console.log(`♻️ Loaded ${files.length} files from previous scan`);
            }
            
            // Apply limit if specified
            if (this.limit) { files = files.slice(0, this.limit); console.log(\`📊 Limited to ${this.limit} files for testing\`);
            }
    ...
    async backupDatabase() { const dbPath = path.join(__dirname, `../music_analyzer.db\`); const backupPath = path.join(__dirname, \`../backup/music_analyzer_${Date.now()}.db`);
        
        await fs.mkdir(path.dirname(backupPath), { recursive: true });
        
        try { await fs.copyFile(dbPath, backupPath); console.log(`✅ Database backed up to: ${path.basename(backupPath)}\`); } catch (error) { console.log(\`⚠️ Could not backup database (might not exist yet)`);
        }
    }
    ...
    async migrateToMainTable() { const db = new sqlite3.Database(; path.join(__dirname, \`../music_analyzer.db`)
        );
        
        return new Promise((resolve, reject) => { // Migrate unique records to main table const sql  = `
            ...
            db.run(sql, [], function(err) { if (err) { console.error(\`❌ Migration error:`, err);
                    reject(err); } else { console.log(`✅ Migrated ${this.changes} new tracks to main table`);
                    db.close();
                    resolve();
                }
            });
    ...
    async createIndexes() {
        ...
        const indexes = [;
            ...
            'CREATE INDEX IF NOT EXISTS idx_title ON audio_files(title)', 'CREATE INDEX IF NOT EXISTS idx_genre ON audio_files(genre)'
        ];
        ...
        return new Promise((resolve, reject) => {
            ...
            indexes.forEach(sql => {
                db.run(sql, [], (err) => { if (err) console.error(`Index error:`, err);
                    completed++;
                     if (completed === indexes.length) { console.log(\`✅ Created ${indexes.length} indexes\`);
                        db.close();
                        resolve();
                    }
                });
    ...
    async loadCheckpoint() {
        ...
        } catch { console.log('💡 No checkpoint found, starting fresh');
        }
    }
    ...
    async generateReport() {
        ...
        const report = {
            timestamp: new Date().toISOString(), duration: `${(duration / 60).toFixed(2)} minutes`,
            stats: {
        ...
        await fs.writeFile(
         ...
         console.log('\n' + '='.repeat(50)); console.log('📊 IMPORT REPORT`); console.log(`=\`.repeat(50)); console.log(\`⏱️ Total time: ${report.duration}`); console.log(`📁 Files scanned: ${report.stats.filesScanned}\`); console.log(\`📚 Metadata extracted: ${report.stats.metadataExtracted}`); console.log(`🎨 Artwork extracted: ${report.stats.artworkExtracted}\`); console.log(\`⚡ Performance: ${report.performance.filesPerSecond} files/sec`); console.log(`❌ Errors: ${report.stats.errors}`);
...
if (require.main === module) {
    ...
    const options = {
        ...
        limit: args.find(a => a.startsWith('--limit='))?.split('=')[1], resume: args.includes('--resume'), workers: parseInt(args.find(a => a.startsWith('--workers='))?.split(`=`)[1] || `1`)
    };
