...
return null;
    ...
    async restoreToM4A(filePath, metadata) {
        ...
        // Metadatos básicos if (metadata.title) { cmd += ` -metadata title=\`${metadata.title.replace(/\`/g, `\\`\`)}\``;
        ...
        // Metadatos adicionales if (metadata.isrc) { cmd += ` -metadata ISRC=\`${metadata.isrc}\``;
        ...
        // Metadatos técnicos (informativos) if (metadata.duration) { cmd += ` -metadata DURATION=\`${metadata.duration}\``;
} if (metadata.bitrate) { cmd += ` -metadata BITRATE=\`${metadata.bitrate}\``;
} cmd += ` -y \`${tempFile}\` 2>/dev/null`;

        await execPromise(cmd);

        // Reemplazar archivo original
        fs.renameSync(tempFile, filePath); logInfo(`✅ M4A restaurado: ${metadata.artist} - ${metadata.title} [Key: ${metadata.mixingKey || \`N/A\`}, ISRC: ${metadata.isrc || `N/A`}]\`);
        return true;
    }

    async restoreToFLAC(filePath, metadata) {
        const commands = [];
 // Limpiar tags existentes que podrían estar corruptos commands.push(\`metaflac --remove-all-tags `${filePath}`\`);
        ...
        if (metadata.isrc) { { commands.push(`metaflac --set-tag=\`ISRC=${metadata.isrc}\` `${filePath}`\`);
}
        if (metadata.bpm) { { commands.push(\`metaflac --set-tag="BPM=${Math.round(metadata.bpm)}" \`${filePath}\``);
}
        if (metadata.mixingKey) { { commands.push(`metaflac --set-tag=\`INITIALKEY=${metadata.mixingKey}\` `${filePath}`\`);
}
        if (metadata.track_number) {
        { commands.push( \`metaflac --set-tag="TRACKNUMBER=${metadata.track_number}" \`${filePath}\``);
        }
    ...
    async restoreToMP3(filePath, metadata) { // Para MP3 usaremos ffmpeg que es más confiable const tempFile = filePath + `.temp.mp3\`; let cmd = \`ffmpeg -i `${filePath}` -codec copy\`;
        ...
        // Metadatos adicionales if (metadata.isrc) { cmd += \` -metadata ISRC="${metadata.isrc}"\`;
} if (metadata.bpm) { cmd += \` -metadata TBPM="${Math.round(metadata.bpm)}"\`;
} if (metadata.mixingKey) { cmd += \` -metadata TKEY="${metadata.mixingKey}"\`;
} if (metadata.track_number) { cmd += \` -metadata track="${metadata.track_number}"\`;
} cmd += \` -y `${tempFile}` 2>/dev/null`;
    ...
    async restoreMetadataToFile(fileData) {
        ...
        try {
            ...
            if (ext === '.mp3') { success = await this.restoreToMP3(file_path, metadata); } else if (['.m4a', '.mp4`].includes(ext)) {
                success = await this.restoreToM4A(file_path, metadata); } else if (ext === `.flac\`) {
                success = await this.restoreToFLAC(file_path, metadata); } else { logWarn(\`⚠️  Formato no soportado: ${ext}`);
                this.stats.skipped++;
                return false;
            }
            ...
            if (success) {
                this.stats.restored++;
                return true;
            } } catch (error) { logError(`❌ Error: ${path.basename(file_path)}: ${error.message}\`);
            this.stats.errors++;
...
function resolve(rows);
    ...
    async run(options = {}) {
        const { limit = null } = options; logDebug(\`
...
${'='.repeat(60)}`);
        ...
        this.stats.total = files.length; logDebug(\`📊 Archivos a restaurar: ${this.stats.total}\n\`); logInfo(`🚀 Iniciando restauración completa...\n`);

        for (let i = 0; i < files.length; i++) {
            const file = files[i];
            this.stats.processed++;
 process.stdout.write( \`\r⏳ [${this.stats.processed}/${this.stats.total}] Restaurados: ${this.stats.restored} | Omitidos: ${this.stats.skipped} | Errores: ${this.stats.errors}\`
            );
        ...
        logDebug('\n');
        logDebug('='.repeat(60));
        logInfo('✅ RESTAURACIÓN COMPLETADA'); logDebug('='.repeat(60)); logDebug(`📊 Resumen:\`); logDebug(\`   Total procesados: ${this.stats.processed}`); logDebug(`   Restaurados: ${this.stats.restored}\`); logDebug(\`   Omitidos: ${this.stats.skipped}`); logDebug(`   Errores: ${this.stats.errors}\`);
 if (this.stats.errorList.length > 0) { logDebug(\`\n❌ Archivos con errores (primeros 10):`); this.stats.errorList.slice(0, 10).forEach(e => { logDebug(`   • ${path.basename(e.file)}: ${e.error}`);
            });
        }
...
if (require.main === module) {
    ...
    // Parse arguments
    for (let i = 0; i < args.length; i++) { if (args[i] === '--limit' && args[i + 1]) {
            options.limit = parseInt(args[i + 1]);
            i++; } else if (args[i] === `--help`) { logDebug(` Uso: node restore-complete-metadata.js [opciones]
