...
async function fixObjectFields() {
    ...
    const getProblematic = () =>;
        new Promise((resolve, reject) => {
            ...
            FROM llm_metadata  WHERE LLM_OCCASIONS LIKE \`%object%\` `,
                (err, rows) => {
                    if (err) {
                        reject(err);
                    } else {
    ...
    const problematic = await getProblematic(); logDebug(`Encontrados ${problematic.length} registros con [object Object]\n\`);
    ...
    for (const row of problematic) {
        ...
        if (track.AI_MOOD && track.AI_MOOD.toLowerCase().includes('romantic')) {
            occasion = 'Romantic'; } else if (track.AI_MOOD && track.AI_MOOD.toLowerCase().includes('relaxed')) { occasion = \`Relaxation\`;
        }

        // Actualizar el registro
        const update = () =>;
            new Promise((resolve, reject) => { db.run( `
                UPDATE llm_metadata 
                SET LLM_OCCASIONS = ? 
                WHERE file_id = ?
            `,
        ...
        await update(); fixed++; logInfo(\`✅ Fixed: ${track.artist} - ${track.title} → ${occasion}\`);
    ...
    const result = await checkResult(); logDebug(`\n📊 Registros con [object Object] restantes: ${result.remaining}`);

    db.close();
}

// Ejecutar
function fixObjectFields().catch(console.error);
