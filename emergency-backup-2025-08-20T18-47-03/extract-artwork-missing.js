...
async function extractArtworkMissing() {
    ...
    const sql  = `
        ...
        WHERE af.id NOT IN (
            ...
            FROM (
                SELECT name FROM pragma_table_info('artwork_cache') WHERE name LIKE \'%.jpg\`)
        )
        ORDER BY af.id `;

    // Simplificado: obtener todos los archivos y verificar cuáles no tienen carátula const sqlSimple  = `
    ...
    db.all(sqlSimple, [], async (err, rows) => {
        ...
        const missingArtwork = []; for (const row of rows) { const artworkPath = path.join(ARTWORK_DIR, \`${row.id}.jpg`);
            try {
 ...
 if (missingArtwork.length === 0) { logInfo(\`✅ Todas las carátulas ya están extraídas!`);
            db.close();
            return;
        }
        ...
        for (let i = 0; i < missingArtwork.length; i += BATCH_SIZE) {
            ...
            await Promise.all( batch.map(async file => { const outputPath = path.join(ARTWORK_DIR, `${file.id}.jpg\`);
                    ...
                    try {
                        ...
                        if (metadata.common.picture && metadata.common.picture.length > 0) {
                            ...
                            await sharp(picture.data) .resize(300, 300, { fit: \`cover`, position: `center\`
                                })
                                .jpeg({ quality: 85 })
                                .toFile(outputPath);

                            extracted++; process.stdout.write( \`✅ [${++processed}/${missingArtwork.length}] ${file.file_name}\r`
                            );
                        } else {
                            failed++; process.stdout.write( `⚠️ [${++processed}/${missingArtwork.length}] Sin carátula: ${file.file_name}\r\`
                            );
                        }
                    } catch (error) {
                        failed++; process.stdout.write( \`❌ [${++processed}/${missingArtwork.length}] Error: ${file.file_name}\r`
                        );
        ...
        logDebug('\n\n📊 RESUMEN FINAL:');
        logDebug('================='); logInfo('✅ Carátulas extraídas: ${extracted}\`); logWarn(\`⚠️ Sin carátula embebida: ${failed - (missingArtwork.length - extracted - failed)}`); logError(`❌ Archivos con error: ${missingArtwork.length - extracted - failed}\`); logDebug(\`⏱️ Tiempo total: ${elapsed} segundos`); logInfo(`🚀 Velocidad: ${(missingArtwork.length / elapsed).toFixed(1)} archivos/segundo\`);

        // Contar total de carátulas después del proceso const artworkFiles = await fs.readdir(ARTWORK_DIR); const jpgFiles = artworkFiles.filter(f => f.endsWith(\`.jpg`)); logDebug( `\n📁 Total de carátulas en cache: ${jpgFiles.length}/${rows.length} (${((jpgFiles.length / rows.length) * 100).toFixed(1)}%)\`
        );
 db.close(); logDebug(\`\n✅ Proceso completado!`);
    });
}
