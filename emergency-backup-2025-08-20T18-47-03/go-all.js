...
(async () => {
 ...
 logDebug(\'🔧 Normalizando datos...\`); execSync(`node normalize-all-fields.js`, { stdio: \`pipe\` });

    // Procesar todos en lotes
    for (let i = 1; i <= batches; i++) { const size = Math.min(batchSize, pending - (i - 1) * batchSize); logDebug(`\n🤖 [Lote ${i}/${batches}] Analizando ${size} tracks...`);
 try { execSync(\`node handlers/normalized-llm-handler.js ${size}\`, {
                stdio: 'inherit'
            });
