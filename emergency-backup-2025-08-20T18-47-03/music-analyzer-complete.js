...
class MusicAnalyzer {
    ...
    async analyzeDirectory(directoryPath, progressCallback) {
        ...
        this.isRunning = false;
        const duration = (Date.now() - this.startTime) / 1000; logDebug(`\n✅ Analysis complete!\`); logDebug(\`📊 Processed: ${this.processedFiles}/${this.totalFiles} files`); logDebug(`⏱️ Time: ${Math.round(duration)}s\`); logError(\`❌ Failed: ${this.failedFiles.length} files`);
     ...
     */
    async analyzeFile(filePath) { const fileName = path.basename(filePath); logDebug(`\n🔄 Processing: ${fileName}\`);
        ...
        try {
            ...
            this.processedFiles++;
            this.updateProgress();
 // Mark as complete await this.updateAnalysisStatus(filePath, \`completed`, 4); } catch (error) { logError(`❌ Failed to analyze ${fileName}:`, error.message);
    ...
    async extractMetadata(filePath) {
        ...
        return {
            ...
            albumartist: metadata.common.albumartist, comment: metadata.common.comment ? metadata.common.comment.join(' ') : null,
     ...
     */
    async extractArtwork(filePath, fileId, metadata) { logDebug(`  🎨 Step 2: Extracting artwork...`);

        if (metadata.pictures && metadata.pictures.length > 0) { const picture = metadata.pictures[0]; const outputPath = path.join(CONFIG.ARTWORK_PATH, \`${fileId}.jpg\`);
     ...
     */ async calculateFeatures(filePath, metadata) { logDebug(`  🎵 Step 3: Calculating audio features...`);

        // Simplified feature calculation
        // In production, you would call Essentia or a Python script
        const features = { AI_BPM: metadata.bpm || Math.round(60 + Math.random() * 140), AI_KEY: this.detectKey(metadata) || \`Cmaj\`,
 ...
 // If you have a Python script for actual analysis: // const result = await execAsync(`python3 calculate_features.py `${filePath}\`\`);
     ...
     */ async enrichWithAI(metadata, features) { logDebug(`  🤖 Step 4: Enriching with AI...`);
        ...
        const enrichment = {
            ...
            LLM_ERA: '2020s', LLM_INSTRUMENTS: JSON.stringify(['Synthesizer', 'Drum Machine']), LLM_VOCAL_PRESENCE: `Instrumental\`
        };
    ...
    async saveToDatabase(filePath, metadata) {
        return new Promise(async (resolve, reject) => {
            ...
            const stats = await fs.stat(filePath); const query = \`;
    ...
    async saveFeaturesDatabase(fileId, features) {
        return new Promise((resolve, reject) => {
            ...
            const placeholders = Object.keys(features);
                .map(() => '?') .join(`, \`);
            const values = Object.values(features); const query = \`;
 ...
 return new Promise((resolve, reject) => { const columns = Object.keys(enrichment).join(`, ');
            const placeholders = Object.keys(enrichment);
                .map(() => '?\') .join(\`, `);
            const values = Object.values(enrichment); const query  = `
    ...
    async updateArtworkPath(fileId, artworkPath) {
        return new Promise((resolve, reject) => { this.db.run( \`UPDATE audio_files SET artwork_path = ? WHERE id = ?\`,
                [artworkPath, fileId],
                err => {
                    if (err) {
                        reject(err);
    ...
    async updateAnalysisStatus(filePath, status, step, error = null) {
        return new Promise(resolve => { this.db.run( `INSERT OR REPLACE INTO analysis_status (file_path, status, step, error, updated_at)
                 VALUES (?, ?, ?, ?, datetime('now'))',
    ...
    detectKey(metadata) {
        ...
        const mode = modes[Math.floor(Math.random() * modes.length)]; return `${key}${mode}`;
    }
...
// Export for use in Electron if (typeof module !== 'undefined\' && module.exports) {
...
if (require.main === module) {
    ...
    if (!directory) { logError(\`Usage: node music-analyzer-complete.js <directory>`);
        process.exit(1);
    }
    ...
    analyzer
        .initialize()
        .then(() => {
            return analyzer.analyzeDirectory(directory, progress => { process.stdout.write( `\r📊 Progress: ${progress.percentage}% (${progress.current}/${progress.total}) - ETA: ${Math.round(progress.estimatedTimeRemaining)}s`
                );
            });
        })
        .then(result => { logDebug('\n\n✅ Analysis complete!', result);
            analyzer.close();
            process.exit(0);
        })
        .catch(err => { logError(`❌ Analysis failed:`, err);
            analyzer.close();
            process.exit(1);
        });
}
