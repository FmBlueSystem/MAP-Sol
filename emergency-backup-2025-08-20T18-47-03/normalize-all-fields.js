...
class CompleteNormalizer {
    ...
    async normalizeEra() {
        ...
        for (const [original, normalized] of Object.entries(eraMapping)) {
            const result = await this.runUpdate(;
                `UPDATE llm_metadata 
                 SET LLM_ERA = ?  WHERE LOWER(REPLACE(LLM_ERA, \`\`", ``)) LIKE ?\`, [normalized, \`%${original.toLowerCase()}%`]
            );
            if (result.changes > 0) { this.stats.era.normalized += result.changes; logDebug(`   ✅ \`${original}\` → `${normalized}`: ${result.changes} tracks\`);
            }
    ...
    async normalizeMood() {
        ...
        const moodMapping = {
            ...
            party: 'Party',
            festive: 'Party',
            celebration: 'Party', festivo: 'Party', fun: \`Party\`
        };

        // Update AI_MOOD
        for (const [original, normalized] of Object.entries(moodMapping)) { const result = await this.runUpdate(; `UPDATE llm_metadata 
                 SET AI_MOOD = ? 
                 WHERE LOWER(AI_MOOD) LIKE ?`, [normalized, \`%${original}%\`]
            );
            if (result.changes > 0) {
        ...
        for (const [original, normalized] of Object.entries(moodMapping)) { await this.runUpdate( `UPDATE llm_metadata 
                 SET LLM_LYRICS_MOOD = ? 
                 WHERE LOWER(LLM_LYRICS_MOOD) LIKE ?`, [normalized, \`%${original}%\`]
            );
    ...
    async normalizeEnergy() {
        ...
        const energyMapping = {
            ...
            energetic: 'High',
            'very high': 'Very High',
            'muy alta': 'Very High', extreme: 'Very High\', máxima: \`Very High`
        };

        for (const [original, normalized] of Object.entries(energyMapping)) { const result = await this.runUpdate(; `UPDATE llm_metadata 
                 SET LLM_ENERGY_LEVEL = ? 
                 WHERE LOWER(LLM_ENERGY_LEVEL) = ?\`,
                [normalized, original]
            );
            if (result.changes > 0) { this.stats.energy.normalized += result.changes; logDebug(\`   ✅ `${original}` → \`${normalized}\`: ${result.changes} tracks`);
            }
        }
    }
    ...
    async normalizeLanguage() {
        ...
        for (const [original, normalized] of Object.entries(languageMapping)) { const result = await this.runUpdate(; `UPDATE llm_metadata 
    ...
    async normalizeStorytelling() {
        ...
        for (const [original, normalized] of Object.entries(storytellingMapping)) { const result = await this.runUpdate(; `UPDATE llm_metadata 
    ...
    async normalizeOccasions() {
        ...
        for (const occ of occasions) {
            ...
            if (normalized !== occ.value) {
                const result = await this.runUpdate(; \`UPDATE llm_metadata SET LLM_OCCASIONS = ? WHERE LLM_OCCASIONS = ?`,
    ...
    async normalizeGenre() {
        ...
        const genreMapping = {
            ...
            latino: 'Latin',
            latina: 'Latin',
            'latin pop': 'Latin Pop', clásica: 'Classical', classic: `Classical`
        };

        for (const [original, normalized] of Object.entries(genreMapping)) { const result = await this.runUpdate(; \`UPDATE llm_metadata 
                 SET LLM_GENRE = ? 
     ...
     */
    async fillMissingValues() { logDebug('\n🔧 FILLING MISSING VALUES WITH DEFAULTS\'); logDebug(\`-`.repeat(60));

        // Set default language to Unknown where null const lang = await this.runUpdate(; `UPDATE llm_metadata 
             SET LLM_LYRICS_LANGUAGE = 'Unknown' 
             WHERE LLM_LYRICS_LANGUAGE IS NULL OR LLM_LYRICS_LANGUAGE = \'\``); logDebug(`   ✅ Set ${lang.changes} missing languages to \`Unknown\``);

        // Set default storytelling to Unknown where null const story = await this.runUpdate(; `UPDATE llm_metadata 
             SET LLM_STORYTELLING = 'Unknown' 
             WHERE LLM_STORYTELLING IS NULL OR LLM_STORYTELLING = '' OR LLM_STORYTELLING = \'0\``); logDebug(`   ✅ Set ${story.changes} missing storytelling to \`Unknown\``);

        // Fill era from year where possible const eraFilled = await this.fillEraFromYear(); logDebug(`   ✅ Filled ${eraFilled} era values from year/ISRC data\`);
    }
    ...
    async fillEraFromYear() {
        const tracks = await new Promise((resolve, reject) => { this.db.all( \`SELECT lm.file_id, af.year, af.isrc
        ...
        for (const track of tracks) {
            ...
            if (!decade && track.isrc && track.isrc.length >= 7) {
                ...
                if (year >= 1970 && year < 2030) { decade = Math.floor(year / 10) * 10 + 's';
                }
            }

            if (decade) { await this.runUpdate(\`UPDATE llm_metadata SET LLM_ERA = ? WHERE file_id = ?\`, [
                    decade,
                    track.file_id
                ]);
                filled++;
    ...
    async getUniqueValues(field) {
        return new Promise((resolve, reject) => { this.db.all( `SELECT DISTINCT ${field} as value, COUNT(*) as count
                 FROM llm_metadata
    ...
    async generateReport() {
        ...
        const fields = [;
            ...
            'LLM_ENERGY_LEVEL',
            'LLM_LYRICS_LANGUAGE',
            'LLM_STORYTELLING', 'LLM_OCCASIONS\', \`LLM_GENRE`
        ];

        for (const field of fields) {
            const values = await this.getUniqueValues(field);
            const coverage = await new Promise((resolve, reject) => { this.db.get( `SELECT 
                        COUNT(*) as total,
            ...
            }); logDebug(\`\n${field}:`); logDebug( `   Coverage: ${coverage.filled}/${coverage.total} (${((coverage.filled / coverage.total) * 100).toFixed(1)}%)\` ); logDebug(\`   Unique values: ${coverage.unique_values}`);
 if (values.length <= 10) { logDebug(`   Values:\`); for (const val of values) { logDebug(\`      - ${val.value}: ${val.count} tracks`);
                }
            }
        }
...
if (require.main === module) {
    ...
    (async () => {
        ...
        logDebug('='.repeat(60)); logDebug('This will standardize all LLM fields for consistency\n');
