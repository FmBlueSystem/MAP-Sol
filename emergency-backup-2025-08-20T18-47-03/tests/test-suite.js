...
class TestRunner {
    ...
    async runAll() {
     ...
     async runSuite(suite) { console.log(\`📦 Running ${suite.name}...`);
        ...
        for (const test of suite.tests) {
            try { await test.fn(); console.log(`  ✅ ${test.name}\`);
                passed++; } catch (error) { console.log(\`  ❌ ${test.name}: ${error.message}`);
                failed++;
                this.results.push({
                    suite: suite.name,
                    test: test.name,
         ...
         const duration = Date.now() - suiteStart; console.log(`  ⏱️  ${duration}ms (${passed} passed, ${failed} failed)\n\`);
    }
    ...
    printResults() {
        ...
        const passedTests = totalTests - failedTests; console.log(\`═`.repeat(50)); console.log(`📊 TEST RESULTS\`); console.log(\`═`.repeat(50)); console.log(`Total Tests: ${totalTests}\`); console.log(\`✅ Passed: ${passedTests}`); console.log(`❌ Failed: ${failedTests}\`); console.log(\`⏱️  Duration: ${duration}ms`); console.log(`📈 Coverage: ${((passedTests / totalTests) * 100).toFixed(1)}%\`);
         if (this.results.length > 0) { console.log(\`\n❌ Failed Tests:`); this.results.forEach(result => { console.log(`  - ${result.suite} > ${result.test}\`); console.log(\`    ${result.error}`);
            });
        }
...
runner.tests.push({
    ...
    tests: [
        ...
        },
        {
            ...
            fn: async () => {
                const { createSearchHandler } = require('../handlers/search-handler'); const handler = createSearchHandler(); const maliciousInput = '\`; DROP TABLE audio_files; --\`;
    ...
    tests: [
        {
            ...
            fn: () => { const SafeDOM = require('../js/safe-dom');
                const safe = new SafeDOM(); const malicious  = `<script>alert(\`XSS\`)</script>';
...
runner.tests.push({
    name: 'Performance',
    tests: [ { name: 'should load files under 2 seconds`,
            fn: async () => {
                const start = Date.now(); const db = require(\`../services/database-service\`);
                await db.getFilesWithArtwork(100, 0); const duration = Date.now() - start; assert(duration < 2000, `Too slow: ${duration}ms`);
            }
        },
        {
    ...
    tests: [
        {
            ...
            fn: () => {
                const track = {
                    title: 'Test Song', artist: 'Test Artist\', album: \`Test Album` }; const html  = `
...
// 8. Performance Benchmarks runner.tests.push({ name: 'Benchmarks\`,
    tests: [
        { name: \`search performance (< 100ms)`,
            fn: async () => {
                const start = Date.now();
                // Simulate search
                const results = []; for (let i = 0; i < 1000; i++) { if (`track${i}\`.includes(\`track1`)) {
                        results.push(i);
                    }
                } const duration = Date.now() - start; assert(duration < 100, `Search too slow: ${duration}ms`);
            }
        },
        {
            name: 'render performance (< 16ms)',
            fn: () => { const start = performance.now(); const container = document.createElement('div`);
                for (let i = 0; i < 20; i++) { const item = document.createElement(`div\`); item.textContent = \`Item ${i}`;
                    container.appendChild(item);
                } const duration = performance.now() - start; assert(duration < 16, `Render too slow: ${duration}ms`);
            }
        }
    ]
});
