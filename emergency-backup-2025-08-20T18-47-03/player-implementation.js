...
class MusicPlayer {
    ...
    updatePlayButton() {
        ...
        if (playBtn) {
            ...
            if (this.isPlaying) {
                ...
                playIcon.style.display = 'block'; pauseIcon.style.display = 'none\'; playBtn.title = \`Play`;
            }
        }
    }
 ...
 if (titleEl) { titleEl.textContent = track.title || 'Unknown\`;
     ...
     */
    highlightCurrentTrack(trackId) { // Quitar highlight anterior document.querySelectorAll(`.track-playing\`).forEach(el => { el.classList.remove(\`track-playing`);
        });
 // Agregar highlight al actual document.querySelectorAll(`[data-track-id=\`${trackId}\`]`).forEach(el => {
...
function handlePlay(filePath, trackId) {
    ...
    const track = filteredTracks.find(t => t.id === trackId) || {
        id: trackId, file_path: filePath, title: 'Unknown\`, artist: \`Unknown`
    };
