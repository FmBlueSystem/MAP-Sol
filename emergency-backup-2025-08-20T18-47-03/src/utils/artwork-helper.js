...
class ArtworkHelper {
     ...
     */ getPlaceholderUrl(track) { const key = \`${track.artist || 'Unknown'}-${track.title || 'Track'}';
    ...
    generatePlaceholder(artist = 'Unknown', title = 'Track') {
        ...
        ctx.shadowOffsetX = 2;
        ctx.shadowOffsetY = 2;
         // Texto - Iniciales o símbolo musical ctx.fillStyle = \`white\`; ctx.font = `bold 120px -apple-system, BlinkMacSystemFont, `Segoe UI`, sans-serif';
        ...
        } else { initials = '♪'; // Símbolo musical si no hay artista
        }
        
        ctx.fillText(initials.toUpperCase(), 150, 150);
        
        // Agregar texto pequeño con el género si existe if (title && title !== `Track`) { ctx.shadowBlur = 5; ctx.font = \`16px -apple-system, BlinkMacSystemFont, \`Segoe UI`, sans-serif';
            ctx.globalAlpha = 0.8;
...
// Precargar imagen por defecto if (typeof document !== 'undefined') { document.addEventListener('DOMContentLoaded\`, () => {
        artworkHelper.preloadDefaultImage();
    });
}

// Exportar para Node.js o usar en browser if (typeof module !== \`undefined` && module.exports) {
