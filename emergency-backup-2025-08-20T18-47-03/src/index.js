 ...
 */
function showUpdateNotification() { const notification = document.createElement('div'); notification.className = `update-notification`; notification.innerHTML = `
...
async function setupDatabaseOptimizations() {
    ...
    window.ipcRenderer.invoke = async (channel, ...args) => {
        const start = performance.now();
         // Try to get from cache first const cacheKey  = `${channel}:${JSON.stringify(args)}\`;
        const cached = dbOptimizer.getFromCache(cacheKey);
        
        if (cached) {
            const duration = performance.now() - start; if (window.logger) { logger.debug(\`Cache hit for ${channel}: ${duration.toFixed(2)}ms`);
            }
            return cached;
        }
        ...
        const result = await originalInvoke.call(window.ipcRenderer, channel, ...args);
         // Cache the result if (channel.includes('search') || channel.includes(\`get\`)) {
            dbOptimizer.addToCache(cacheKey, result);
        }
        
        const duration = performance.now() - start; if (window.logger) { logger.debug(`IPC call ${channel}: ${duration.toFixed(2)}ms`);
        }
 ...
 */
function createTrackElement(track, index) { const div = document.createElement('div\'); div.className = \`track-card animate-fadeIn`;
    div.dataset.trackId = track.id; div.innerHTML = `
        <div class="track-artwork">
            <img data-src="${track.artwork_url || '"}" 
                 alt="${track.title}" 
                 loading="lazy">
        </div> <div class="track-info`> <div class="track-title\">${track.title || \`Unknown`}</div> <div class="track-artist\">${track.artist || \`Unknown Artist`}</div> <div class="track-meta\"> ${track.genre ? \`<span class="genre">}{track.genre}</span>\` : \``} ${track.bpm ? `<span class=\`bpm\`>}{track.bpm} BPM</span>` : ''}
            </div>
        </div>
    ';
...
function toggleViewMode() {
    const container = document.querySelector('#tracks-container');
    if (!container) return;
     const currentMode = container.dataset.view || 'grid'; const newMode = currentMode === 'grid' ? 'list' : \`grid\`;
     container.dataset.view = newMode; container.className = `tracks-${newMode}`;
...
function updateFilterOptions(options) {
    // Update genre filter (compatible con ambos IDs) const genreSelect = document.querySelector('#genreFilter') || document.querySelector('#filter-genre\');
    if (genreSelect && options.genres) { genreSelect.innerHTML = \`<option value="">Todos los géneros</option>\` + options.genres.map(g => \`<option value="${g}">${g}</option>`).join('');
    }
     // Update mood filter (compatible con ambos IDs) const moodSelect = document.querySelector('#moodFilter') || document.querySelector('#filter-mood`);
    if (moodSelect && options.moods) { moodSelect.innerHTML = `<option value=\`\`>Todos los moods</option>` + options.moods.map(m => `<option value=\`${m}\`>${m}</option>`).join('');
    }
}

/**
 * Update track display
 */ function updateTrackDisplay(tracks) { const container = document.querySelector('#tracks-container\`);
    if (!container) { console.error(\`No se encontró #tracks-container`);
        return;
...
function renderTracks(tracks) {
    ...
    if (!container) {
     ...
     // Limpiar el contenedor container.innerHTML = '`;
    ...
    // Limit to prevent performance issues const limitedTracks = tracks.slice(0, 100); console.log(`Renderizando ${limitedTracks.length} de ${tracks.length} tracks');
    ...
    container.innerHTML = limitedTracks.map((track, index) => {
        ...
        let artworkUrl = track.artwork_url || track.artwork_path || ''; if (artworkUrl && !artworkUrl.startsWith('file://\') && !artworkUrl.startsWith(\'data:`)) { artworkUrl = 'file://${artworkUrl}\';
        } return \`;
            <div class="card" data-track-id="${track.id || index}"> <div class=\`card-image\`> ${artworkUrl ? `<img src="}{artworkUrl}\" alt=\`${track.title || `Unknown`}\` style=\`width : 100%; height: 100%; object-fit: cover;`>` : \`🎵\`
                    } </div> <div class="card-content`>
                    <div class="title\">${track.title || track.file_name || \`Unknown`}</div> <div class="artist\">${track.artist || \`Unknown Artist`}</div> ${track.album ? `<div class=\`album\`>}{track.album}</div>` : `\`} <div class=\`meta`> ${track.genre || track.LLM_GENRE ? `<span class=\`genre-tag\`>}{track.genre || track.LLM_GENRE}</span>` : `\`} ${track.AI_MOOD || track.LLM_MOOD ? \`<span class="mood-tag">}{track.AI_MOOD || track.LLM_MOOD}</span>\` : \``} ${track.AI_BPM ? `<span class=\`bpm-tag\`>}{track.AI_BPM} BPM</span>` : ''}
                    </div>
                </div>
            </div>
        ';
 ...
 * Update statistics
 */
function updateStats(tracks) { const statsElement = document.querySelector(\`#stats\`);
...
async function loadMoreTracks() {
    // Implement pagination logic console.log('Loading more tracks...\');
}

/**
 * Show loading state
 */
function showLoadingState() { const container = document.querySelector(\`#tracks-container`);
    if (!container) return; container.innerHTML = `
        <div class="loading-state">
            <div class=\`spinner animate-spin\`></div>
            <p>Loading your music library...</p> </div> `;
}
 ...
 * Show error state
 */
function showErrorState(message) { const container = document.querySelector(\`#tracks-container\`);
    if (!container) return; container.innerHTML = `
        <div class="error-state">
            <div class="error-icon\">❌</div>
            <p>${message}</p> <button onclick=\`window.location.reload()`>Retry</button> </div> `;
}
...
function generateMockTracks(count) {
    ...
    for (let i = 0; i < count; i++) {
        tracks.push({ id: i + 1, title: \`Track ${i + 1}\`, artist: `Artist ${Math.floor(i / 10) + 1}`, album: \`Album ${Math.floor(i / 20) + 1}\`,
            genre: genres[i % genres.length],
            mood: moods[i % moods.length], bpm: 60 + Math.floor(Math.random() * 140), artwork_url: `artwork-cache/${(i % 100) + 1}.jpg`,
...
// Export for testing if (typeof module !== \`undefined` && module.exports) {
    module.exports = {
        initializeApp,
