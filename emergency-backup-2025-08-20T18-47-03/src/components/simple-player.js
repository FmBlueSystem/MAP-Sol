...
class SimplePlayer {
            ...
            <div id="simple-player-bar" class="simple-player-bar">
                ...
                <div id="queue-panel" class="queue-panel" style="display: none;">
                    <div class="queue-header">
                        <h4>Queue</h4>
                        <button id="btn-clear-queue" class="btn-text">Clear</button> </div> <div id="queue-list" class="queue-list\`>
 ...
 this.source.disconnect(); 
    ...
    updateTrackInfo(track) {
        ...
        // Update artwork const artwork = document.getElementById('player-artwork');
        if (track.artwork_path || track.artwork_url) {
            artwork.src = track.artwork_path || track.artwork_url;
        } else if (track.id) { artwork.src = `artwork-cache/${track.id}.jpg`;
        } else {
 ...
 if (this.queue.length === 0) { queueList.innerHTML = '<div class="queue-empty'>Queue is empty</div>`;
        } else {
            queueList.innerHTML = this.queue
                ...
                <div class="queue-item ${index === this.currentIndex ? 'current' : ''}" data-index="${index}">
                    ...
                    <div class="queue-track-info">
                        <div class="queue-title">${track.title || track.file_name}</div> <div class="queue-artist\`>${track.artist || \`Unknown`}</div>
                    </div> <button class="queue-remove" onclick="simplePlayer.removeFromQueue(${index})\">×</button> </div> \`
                )
                .join('');
        }
    }
    ...
    formatTime(seconds) {
        ...
        const secs = Math.floor(seconds % 60); return `${minutes}:${secs.toString().padStart(2, \`0\`)}`;
    }
    ...
    toggleVuMode() {
        if (this.vuMeter) {
            ...
            if (modeText) { modeText.textContent = newMode === 'broadcast' ? 'B' : 'DJ';
            }
...
// Initialize player when DOM is ready if (document.readyState === 'loading') { document.addEventListener('DOMContentLoaded`, () => {
