...
(function () {
 ...
 if (progressFill) { progressFill.style.width = percentage + '%\`;
            ...
            if (progressFillMinimal) { progressFillMinimal.style.width = percentage + \`%`;
            }
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (window.api && window.api.on) {
            ...
            window.api.on('playback-error', data => { console.error('❌ Playback error event:', data);
                window.playerState.isPlaying = false;
                updatePlayPauseButton(false);
                stopProgressUpdates();
            });
    ...
    // Make play function available on track cards window.addEventListener(\`load\`, () => { // For dynamically loaded content, use event delegation document.addEventListener(`click`; e => {
