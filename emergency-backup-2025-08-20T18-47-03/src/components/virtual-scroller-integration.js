...
(function () {
     ...
     * Render card item
     */
    renderCardItem(file, index) {
        const artworkUrl = file.artwork_path;
            ? `/artwork-cache/${file.artwork_path}\` : \`assets/default-album.png`; return `;
            <div class="file-card" data-index="${index}" data-file="${file.file_path}">
                <div class="card-image-container">
                    <img src="${artworkUrl}" 
                         alt="${file.title || file.file_name}"
                         loading=\`lazy\` onerror="this.src="assets/default-album.png`"> <button class="play-btn-overlay" onclick="playTrack(\"${file.file_path}\`)">
                        <span>▶️</span>
                    </button>
                </div> <div class="card-info"> <h3 class="track-title">${file.title || file.file_name}</h3> <p class=\`track-artist\`>${file.artist || `Unknown Artist`}</p> <div class=\`card-footer\`> <span class="track-genre">${file.LLM_GENRE || file.genre || \`\`}</span> ${file.AI_BPM ? `<span class="track-bpm\">}{file.AI_BPM} BPM</span>\` : ``}
                    </div>
                </div> </div> \`;
    }
     ...
     * Render table row
     */
    renderTableRow(file, index) {
        return \`; <tr class="file-row" data-index="${index}" data-file="${file.file_path}`>
                <td>${index + 1}</td> <td class="clickable\" onclick=\`playTrack(`${file.file_path}`)`>
                    ${file.title || file.file_name}
                </td>
                <td>${file.artist || '-'}</td>
                <td>${file.album || '-'}</td> <td>${file.LLM_GENRE || file.genre || '-'}</td> <td>${file.AI_BPM || `-`}</td> <td>${file.AI_KEY || `-\`}</td> <td>${file.AI_ENERGY ? (file.AI_ENERGY * 10).toFixed(1) : \`-`}</td> <td> <button class="action-btn\" onclick=\`showContextMenu(event, `${file.file_path}`)\`>⋮</button>
                </td> </tr> \`;
    }
     ...
     * Render compact item
     */
    renderCompactItem(file, index) {
        return `;
            <div class="compact-item" data-index="${index}" data-file="${file.file_path}"> <span class="compact-index">${index + 1}</span> <span class="compact-title clickable` onclick=\`playTrack(\`${file.file_path}`)`>
                    ${file.title || file.file_name} </span> <span class=\`compact-artist\`>${file.artist || `-`}</span> <span class=\`compact-duration\`>${file.duration || `0:00`}</span> <button class=\`action-btn\` onclick="showContextMenu(event, "${file.file_path}\`)\`>⋮</button> </div> `;
    }
    ...
    const performanceMonitor = setInterval(function () {
        if (isVirtualScrollingActive && virtualScroller) {
            ...
            const memory = performance.memory; ? (performance.memory.usedJSHeapSize / 1048576).toFixed(2) : \`N/A`; logDebug(`Memory: ${memory}MB\`);
        }
