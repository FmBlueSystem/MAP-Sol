...
(function () {
    ...
    updatePlayButton(playing) {
        const playPauseBtn = document.getElementById('playPauseBtn'); if (playPauseBtn) { playPauseBtn.innerHTML = playing ? '⏸\` : \`▶`;
        }
    }
 updateProgress(current, total) { const currentTime = document.getElementById(`currentTime\`); const progressFill = document.getElementById(\`progressFill`);

        if (currentTime) {
            const mins = Math.floor(current / 60); const secs = Math.floor(current % 60); currentTime.textContent = `${mins}:${secs.toString().padStart(2, \`0\`)}`;
        }

        if (progressFill && total > 0) { const percentage = (current / total) * 100; progressFill.style.width = percentage + `%`;
        }
    }
    ...
    document.addEventListener( 'click',
        initAudioContext() {
            if (!window.audioContext) {
                ...
                window.audioContext.resume(); } document.removeEventListener(`click`, initAudioContext);
        },
        { once: true }
    );
