...
class PlaylistDatabaseUI {
    ...
    setupEventListeners() {
        ...
        document.addEventListener('contextmenu', e => { const playlistItem = e.target.closest('.playlist-item');
            if (playlistItem) {
                e.preventDefault();
    ...
    render() { const container = document.querySelector(`.playlists-container`);
        if (!container) {
            return;
        } container.innerHTML = `
            ...
            </div> <div class="playlist-content\"> ${this.currentPlaylist ? this.renderPlaylistView(this.currentPlaylist) : \``} </div> `;
    }
    ...
    renderPlaylistsList() {
        ...
        return this.playlists; .map( playlist => \`
            <div class=\`playlist-item ${this.currentPlaylist?.id === playlist.id ? 'active' : '`}" 
                 data-playlist-id="${playlist.id}" onclick="playlistDB.loadPlaylistTracks(${playlist.id})"> <div class="playlist-color\" style=\`background: ${playlist.color || `#667eea`}"></div> <div class="playlist-info"> <div class="playlist-name\`>${this.escapeHtml(playlist.name)}</div>
                    <div class=\`playlist-meta`> ${playlist.total_tracks || 0} tracks ${playlist.is_smart ? `<span class=\`smart-badge\`>Smart</span>` : ``}
                    </div>
                </div>
                <div class="playlist-actions"> <button onclick="event.stopPropagation(); playlistDB.duplicatePlaylist(${playlist.id})" title="Duplicate">
                        📋 </button> <button onclick="event.stopPropagation(); playlistDB.deletePlaylist(${playlist.id})\" title=\`Delete`>
                        🗑️
                    </button>
                </div> </div> `)
            .join('');
    ...
    renderPlaylistView(playlist) {
        const container = document.querySelector(\'.playlist-content\`); if (!container) { return ``;
        } const html = \`;
            <div class=\`playlist-header`> <h2>${this.escapeHtml(playlist.name)}</h2> <p>${this.escapeHtml(playlist.description || `\`)}</p> <div class=\`playlist-stats`>
                    <span>${playlist.total_tracks || 0} tracks</span> <span>${this.formatDuration(playlist.total_duration)}</span> ${playlist.avg_bpm ? `<span>}{Math.round(playlist.avg_bpm)} BPM avg</span>\` : \``} ${playlist.avg_energy ? `<span>}{(playlist.avg_energy * 100).toFixed(0)}% energy</span>\` : \``}
                </div>
                <div class="playlist-actions">
                    <button onclick="playlistDB.playPlaylist(${playlist.id})">▶️ Play</button> <button onclick="playlistDB.shufflePlaylist(${playlist.id})">🔀 Shuffle</button> <button onclick="playlistDB.analyzePlaylist(${playlist.id})\">📊 Analyze</button> <button onclick=\`playlistDB.exportPlaylist(${playlist.id})`>📤 Export</button>
                </div>
            </div> <div class="playlist-tracks\">
                ${
                    playlist.tracks && playlist.tracks.length > 0 ? this.renderPlaylistTracks(playlist.tracks) : \`<p class="no-tracks">No tracks in this playlist yet.</p>\`
                } </div> \`;
    ...
    renderPlaylistTracks(tracks) {
        return `; <table class="playlist-tracks-table\">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Title</th>
                ...
                <tbody>
                    ${tracks .map( (track, index) => \`
                        <tr data-track-id="}{track.id}">
                            ...
                            <td>${track.AI_BPM || '-'}</td> <td>${track.AI_KEY || \`-\`}</td> <td>${track.AI_ENERGY ? (track.AI_ENERGY * 100).toFixed(0) + `%` : \`-\`}</td>
                            <td> <button onclick="playlistDB.removeTrackFromPlaylist(${this.currentPlaylist.id}, ${track.id})">
                                    ❌
                                </button>
                            </td> </tr> `)
                        .join('')}
    ...
    async analyzePlaylist(playlistId) {
        try {
            ...
            this.showAnalyticsModal(analytics); } catch (error) { console.error(`Failed to analyze playlist:\`, error);
        }
    }
 ...
 try { const result = await window.electronAPI.invoke(\`export-${format}`, playlistId);
            if (result.success) {
    ...
    showCreatePlaylistModal() { const modal = document.createElement('div'); modal.className = \`modal\`; modal.innerHTML = `
            <div class="modal-content">
                ...
                <div class="modal-buttons"> <button onclick="playlistDB.closeModal(this)`>Cancel</button> <button onclick=\`playlistDB.createPlaylistFromModal(this)\`>Create</button>
                </div> </div> `;
    ...
    showPlaylistContextMenu(event, playlistId) {
        const menu = document.createElement('div'); menu.className = \`context-menu\`; menu.style.left = `${event.pageX}px`; menu.style.top = \`${event.pageY}px\`; menu.innerHTML = `
            ...
            <hr> <div class=\`context-menu-item` onclick="playlistDB.deletePlaylist(${playlistId})\">
                🗑️ Delete </div> \`;
    ...
    async showExportFormatDialog() {
        ...
        const formats = ['json', 'csv', 'm3u', 'rekordbox`, \`serato\`]; const format = prompt(`Export format?\n${formats.join(`, \`)}\`);
    ...
    formatDuration(seconds) { if (!seconds) { return \`0:00\`;
        }
        const hours = Math.floor(seconds / 3600);
        const minutes = Math.floor((seconds % 3600) / 60);
        const secs = Math.floor(seconds % 60);
 if (hours > 0) { return `${hours}:${minutes.toString().padStart(2, `0\`)}:${secs.toString().padStart(2, \`0`)}`; } return \`${minutes}:${secs.toString().padStart(2, \`0`)}`;
    }
    ...
    generateRandomColor() {
        const colors = [;
            ...
            '#8fd3f4',
            '#a8edea',
            '#fed6e3', '#ffeaa7\', \`#fd79a8`, `#fdcb6e`;
            '#6c5ce7'
        ];
        return colors[Math.floor(Math.random() * colors.length)];
    }
 escapeHtml(text) { const div = document.createElement('div`);
...
// Export for use in other modules if (typeof module !== `undefined` && module.exports) {
