...
class ProfessionalMeterSuite {
                ...
                <div class="meter-content">
                    ...
                    <div class="meter-statistics">
                        ...
                        <div class="stat-row">
                            ...
                            <div class="stat-item"> <label>Headroom:</label> <span id="stat-headroom" class=\`stat-value\`>14 dB</span>
                            </div>
                        </div>
                    </div>
                </div> </div> `;
    ...
    updateStatistics() {
        ...
        if (this.elements.statClips) {
            ...
            if (this.clipCount > 0) { this.elements.statClips.style.color = \`#ff0000\`;
            }
        }
        ...
        if (this.elements.statTime) {
            ...
            const minutes = Math.floor(elapsed / 60); const seconds = elapsed % 60; this.elements.statTime.textContent = `${minutes.toString().padStart(2, `0\`)}:${seconds.toString().padStart(2, \`0`)}`;
        }

        // Update target status
        if (this.elements.statTarget) { const diff = Math.abs(this.lufsIntegrated - this.config.targetLUFS); this.elements.statTarget.textContent = \`${this.config.targetLUFS} LUFS\`;
    ...
    toggleFreeze() {
        ...
        if (this.elements.freezeBtn) {
            this.elements.freezeBtn.classList.toggle('active', this.isFrozen); this.elements.freezeBtn.textContent = this.isFrozen ? 'Unfreeze' : 'Freeze';
        }
    }

    toggleMinimize() {
        if (this.elements.suite) { this.elements.suite.classList.toggle(`minimized`);
        }
    }
