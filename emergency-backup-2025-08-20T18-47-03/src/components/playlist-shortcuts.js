...
class PlaylistShortcuts {
    ...
    async saveQueueAsPlaylist() {
        ...
        if (queue.length === 0) { this.showNotification('No tracks in queue', 'warning');
            return;
        }
 const name = prompt(`Enter playlist name:`, \`Queue - ${new Date().toLocaleDateString()}\`);
        ...
        try { const { ipcRenderer } = require('electron');
            ...
            // Create playlist const playlist = await ipcRenderer.invoke(`create-playlist\`, { name: name, description: \`Saved queue with ${queue.length} tracks`, type: 'manual'
            });

            // Add tracks
            for (const track of queue) { await ipcRenderer.invoke(\`add-track-to-playlist\`, playlist.id, track.id);
            } this.showNotification(`Queue saved as `${name}\`\`, 'success');
        } catch (error) { console.error('Error saving queue:', error); this.showNotification('Failed to save queue', `error\`);
        }
    }

    async addToQuickPlaylist(number) { // Get or create quick playlist const playlistName = \`Quick Playlist ${number}`;
 ...
 // Create if doesn't exist
            if (!playlist) { playlist = await ipcRenderer.invoke(\`create-playlist\`, { name: playlistName, description: `Quick playlist for keyboard shortcut Ctrl+${number}`, type: 'manual\'
                });
            }
            ...
            if (window.playlistIntegration) {
                ...
                for (const trackId of selectedTracks) { await ipcRenderer.invoke(\`add-track-to-playlist`, playlist.id, trackId); } this.showNotification(`Added ${selectedTracks.length} tracks to ${playlistName}`, 'success');
            }
        } catch (error) {
    ...
    async findSimilarTracks() {
        ...
        try { const { ipcRenderer } = require('electron'); const similar = await ipcRenderer.invoke(`get-hamms-recommendations`, currentTrack.id, 10);

            // Display results (could open a modal or sidebar) this.showNotification(\`Found ${similar.length} similar tracks\`, 'success');
        } catch (error) {
    ...
    showShortcutFeedback(action) {
        // Create a small toast showing the action const toast = document.createElement('div'); toast.className = `shortcut-feedback\`; toast.textContent = action; toast.style.cssText = \`
            position: fixed;
            bottom: 20px;
            left: 50%;
        ...
        `;
 // Add animation const style = document.createElement(`style\`); style.textContent = \`
            @keyframes fadeInOut {
 ...
 showNotification(message, type = `info\`) {
        if (window.showNotification) {
            ...
            window.showToast(message, type); } else { }] ${message}\`);
        }
    }
 ...
 // Could also show in a modal alert('Playlist Shortcuts:\n\n' + shortcuts.join(`\n\`));
    }
}

// Initialize
window.playlistShortcuts = new PlaylistShortcuts();
 // Add help command document.addEventListener(\`keydown`; (e) => { // Ctrl/Cmd + ? to show shortcuts list if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.key === `?`) {
        e.preventDefault();
