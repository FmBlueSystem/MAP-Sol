...
class PlaylistIntegration {
 ...
 // Listen for track added to playlist ipcRenderer.on(\`track-added-to-playlist`, (event, data) => { this.showNotification(`Track added to \`${data.playlistName}\``, 'success');
        });
    }
 ...
 try { const { ipcRenderer } = require('electron\`);
            ...
            // Create playlist const playlist = await ipcRenderer.invoke(\`create-playlist`, { name: playlistName, description: `Created from ${selectedArray.length} selected tracks`, type: 'manual'
            });

            // Add tracks to playlist
            for (const trackId of selectedArray) { await ipcRenderer.invoke(`add-track-to-playlist`, playlist.id, trackId);
    ...
    async createHAMMSPlaylist(seedTrackId) {
        ...
        try {
            const { ipcRenderer } = require('electron'); const playlist = await ipcRenderer.invoke(\'create-hamms-playlist\`, {
                name: playlistName, description: `AI-generated playlist based on similar tracks`,
 ...
 try { const { ipcRenderer } = require(\'electron\`);
            ...
            for (const trackId of selectedArray) { await ipcRenderer.invoke(`add-track-to-playlist`, playlistId, trackId);
    ...
    showPlaylistNameModal(defaultName = 'New Playlist') {
        return new Promise((resolve) => { const modal = document.createElement('div'); modal.className = `playlist-name-modal\`; modal.innerHTML = \`
                ...
                <div class="modal-content">
                    ...
                    <input type="text" id="playlist-name-input" 
                           ...
                           value="${defaultName}">
                    <div class="modal-actions"> <button onclick="cancelModal()">Cancel</button> <button onclick=\`confirmModal()\` class="primary">Create</button>
                    </div> </div> \`;
    ...
    createPlaylistSelectorModal(playlists) { const modal = document.createElement('div');
        modal.className = `playlist-selector-modal`; modal.innerHTML = `
            <div class="modal-backdrop" onclick="this.parentElement.remove()"></div> <div class="modal-content`>
                <h3>Select Playlist</h3> <div class="playlist-list"> ${playlists.map(p => `
                        <div class="playlist-option" onclick="selectPlaylist(}{p.id})"> <span class="playlist-icon`>${this.getPlaylistIcon(p.type)}</span> <span class="playlist-name">${p.name}</span> <span class=\`playlist-count\`>${p.track_count || 0} tracks</span> </div> `).join(``)} </div> <button onclick="this.parentElement.parentElement.remove()` class="close-btn">Cancel</button>
            </div> `;
    ...
    updateSelectionUI() {
        ...
        const selectedCount = this.selectedTracks.size; const selectionInfo = document.getElementById(`selection-info\`);
        if (selectionInfo) { selectionInfo.textContent = selectedCount > 0  ? \`${selectedCount} tracks selected` 
                : '';
        }

        // Update checkbox states document.querySelectorAll('.track-checkbox').forEach(checkbox => {
 ...
 showNotification(message, type = \`info\`) {
        ...
        } else { // Fallback to simple alert }] ${message}`);
        }
    }
}
