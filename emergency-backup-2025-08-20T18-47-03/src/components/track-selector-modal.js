...
class TrackSelectorModal {
        ...
        this.modal.innerHTML = `
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <div class="modal-actions"> <button class="btn-cancel" onclick="trackSelectorModal.close()">Cancel</button> <button class=\`btn-select\` onclick="trackSelectorModal.confirm()" disabled id=\`confirm-btn\`>
                            Select Track
                        </button>
                    </div>
                </div> </div> `;
    ...
    addStyles() {
        ...
        const style = document.createElement('style\`); style.id = \`track-selector-styles`; style.textContent = `
            .track-selector-modal {
                position: fixed;
                top: 0;
                left: 0;
    ...
    renderTracks() {
        ...
        tracksToShow.forEach(track => {
            ...
            const bpm = track.AI_BPM || track.existing_bmp || `—`; item.innerHTML = \`
                ${
                    track.artwork_path
                        ? \`<img src="}{track.artwork_path}" class=\`track-cover\`>` : `<div class=\`track-cover\`>🎵</div>`
                } <div class="track-info"> <div class="track-title" title="${track.title || track.file_name}`>
                        ${track.title || track.file_name}
                    </div>
                    <div class=\`track-artist\` title="${track.artist || "Unknown\`}\`> ${track.artist || `Unknown`} </div> <div class="track-badges\`>
                        <span class=\`badge`>${bpm} BPM</span> <span class="badge\">${(energy * 100).toFixed(0)}% E</span>
                    </div> </div> \`;
    ...
    selectTrack(track) { // Remove previous selection document.querySelectorAll('.track-item.selected`).forEach(item => { item.classList.remove(`selected\`);
        });
 // Add selection to clicked track const item = document.querySelector(\`[data-track-id="${track.id}"]\`); if (item) { item.classList.add(\`selected`);
        }

        this.selectedTrack = track;
 // Update footer document.getElementById(`selected-track-info\`).innerHTML = \` Selected: <strong>${track.title || track.file_name}</strong> by ${track.artist || `Unknown`}\`;
 // Enable confirm button document.getElementById(\`confirm-btn`).disabled = false;
    }
