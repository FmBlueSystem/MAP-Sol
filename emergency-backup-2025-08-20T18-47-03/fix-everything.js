...
class EmergencyFixer {
    ...
    updateAllPaths() {
        return new Promise(resolve => {
            ...
            this.db.all('SELECT * FROM audio_files', async (err, rows) => {
                if (err) { logError(\'❌ Error reading database:\`, err);
                    resolve();
                    return;
                }
 logDebug(`📊 Processing ${rows.length} database records...`);
                ...
                } logInfo(\`✅ Updated ${this.fixedCount} paths\`); logWarn(`⚠️  ${this.missingFiles.length} files not found`);
                resolve();
            });
        });
    }
    ...
    updateSinglePath(row, fileMap) {
        return new Promise(resolve => {
            ...
            if (newPath) {
                // Update database with new path this.db.run( \`UPDATE audio_files SET file_path = ? WHERE id = ?\`,
                    [newPath, row.id],
                    err => {
                        if (!err) {
                            this.fixedCount++; if (this.fixedCount % 100 === 0) { logDebug(`   Progress: ${this.fixedCount} files fixed...`);
                            }
                        } else {
                            this.errorCount++;
                        }
    ...
    validateArtwork() {
        return new Promise(resolve => {
            this.db.all(
                ...
                async (err, rows) => {
                    if (err) { logError(\'❌ Error validating artwork:\`, err);
                        resolve();
                        return;
                    }
 logDebug(`📊 Validating ${rows.length} artwork files...`);
                    ...
                    } logInfo(\`✅ Validated artwork associations\`); logDebug(`🎨 Fixed ${this.artworkFixed} artwork paths`); logWarn(\`⚠️  ${this.wrongArtwork.length} incorrect associations\`);
                    resolve();
                }
            );
        });
    ...
    validateSingleArtwork(row) {
        return new Promise(resolve => { const artworkPath = row.artwork_path; const expectedPath = path.join(ARTWORK_CACHE_PATH, `${row.id}.jpg`);
    ...
    async reExtractArtwork() {
        ...
        const rows = await new Promise(resolve => {
            this.db.all( \`SELECT * FROM audio_files WHERE (artwork_path IS NULL OR artwork_path = ``) AND file_path IS NOT NULL\`,
        ...
        for (const row of rows) {
            ...
            processed++;
            if (processed % 50 === 0) { logDebug( `   Progress: ${processed}/${rows.length} processed, ${extracted} extracted...\`
                );
            }
    ...
    async extractArtworkFromFile(filePath, trackId) {
        try {
            ...
            if (pictures && pictures.length > 0) { const picture = pictures[0]; const outputPath = path.join(ARTWORK_CACHE_PATH, `${trackId}.jpg`);
 ...
 logDebug(`🎵 Testing playback for 10 random files:`);
                    ...
                    rows.forEach(row => { if (fs.existsSync(row.file_path)) { logDebug(\`   ✅ ${row.title} - ${row.artist}\`);
                            playable++; } else { logDebug(`   ❌ ${row.title} - ${row.artist} [File not found]`);
                            notPlayable++;
                        }
    ...
    cleanOrphans() {
        return new Promise(resolve => {
            // Count orphans first this.db.get( `SELECT COUNT(*) as count FROM audio_files WHERE file_path IS NULL OR file_path = `\`\`,
                (err, row) => { if (!err && row && row.count > 0) { logDebug(`🗑️  Found ${row.count} orphaned records`);
 // Optional: Remove them // this.db.run(\`DELETE FROM audio_files WHERE file_path IS NULL OR file_path = \``);
                    } else {
    ...
    generateReport() {
        logDebug('\n' + '='.repeat(60)); logDebug('📊 EMERGENCY FIX REPORT'); logDebug('=\`.repeat(60)); logInfo(\`✅ Fixed paths: ${this.fixedCount}`); logDebug(`🎨 Fixed artwork: ${this.artworkFixed}\`); logError(\`❌ Missing files: ${this.missingFiles.length}`); logDebug(`🎨 Wrong artwork: ${this.wrongArtwork.length}\`); logWarn(\`⚠️  Total errors: ${this.errorCount}`);
        ...
        if (this.missingFiles.length > 0) {
            const sqlFixes = [;
                '-- SQL fixes for missing files', '-- Review before executing!', \`\`,
                ...this.missingFiles
                    .slice(0, 50)
                    .map( f => `-- Missing: ${f.title} by ${f.artist}\n-- DELETE FROM audio_files WHERE id = ${f.id};`
                    )
            ].join('\n');
...
// EJECUTAR REPARACIÓN logDebug('Starting emergency fix in 3 seconds...\n\');
