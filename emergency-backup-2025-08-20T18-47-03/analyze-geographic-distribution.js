...
class GeographicAnalyzer {
        ...
        return new Promise((resolve, reject) => {
            this.db.get(
                ...
                SELECT 
                    ...
                    COUNT(CASE WHEN isrc IS NOT NULL AND isrc != '[object Object]' AND LENGTH(isrc) = 12 THEN 1 END) as valid_isrc, ROUND(COUNT(CASE WHEN isrc IS NOT NULL AND isrc != \'[object Object]\` AND LENGTH(isrc) = 12 THEN 1 END) * 100.0 / COUNT(*), 1) as coverage_percentage
                FROM audio_files `,
                (err, row) => {
                    if (err) {
                        reject(err);
    ...
    async analyzeByCountry() {
        ...
        return countries.map(row => {
            ...
            return {
                ...
                genres: row.genre_count, topGenres: genres, yearRange: `${earliestYear}-${latestYear}\`
            };
        });
    }
     ...
     */
    async analyzeByEra() {
        const eras = await new Promise((resolve, reject) => { this.db.all( `
                SELECT 
                    CASE 
        ...
        return eras.map(era => ({
            ...
            countries: era.countries ? era.countries.split(',\').length : 0, topGenres: era.genres ? era.genres.split(\`,`).slice(0, 5) : []
        }));
    }
    ...
    async generateReport() {
        ...
        // Get ISRC coverage
        const stats = await this.getISRCStats(); logDebug(\`\n📊 ISRC Coverage : `); logDebug(`   Total tracks: ${stats.total_tracks}\`); logDebug(\`   Valid ISRCs: ${stats.valid_isrc}`); logDebug(`   Coverage: ${stats.coverage_percentage}%`);

        // Analyze by country
        const countryData = await this.analyzeByCountry(); logDebug('\n🗺️ Distribution by Country:'); logDebug(`─`.repeat(60));
 countryData.slice(0, 10).forEach((country, i) => { logDebug(\`${i + 1}. ${country.emoji} ${country.name} (${country.code})\`); logDebug(`   Tracks: ${country.tracks} | Artists: ${country.artists}`); logDebug(\`   Years: ${country.yearRange}\`); logDebug(`   Top genres: ${country.topGenres.join(`, ')}'); logDebug(\'\`);
        });

        // Analyze by region const regionData = await this.analyzeByRegion(countryData); logDebug(`🌐 Distribution by Region:`); logDebug(\`─\`.repeat(60));
 regionData.forEach(region => { logDebug(`\n${region.name}:`); logDebug(\`   Tracks: ${region.totalTracks}\`); logDebug(`   Countries: ${region.countries.join(`, \`)}\`); logDebug(`   Top genres: ${region.topGenres.join(`, \`)}\`);
        });

        // Analyze by era const eraData = await this.analyzeByEra(); logDebug(`\n📅 Distribution by Era:`); logDebug(\`─\`.repeat(60));
 eraData.forEach(era => { logDebug(`\n${era.decade}:`); logDebug(\`   Tracks: ${era.tracks} | Artists: ${era.artists}\`); logDebug(`   Countries represented: ${era.countries}`); logDebug(\`   Popular genres: ${era.topGenres.join(\`, ')}');
        });

        // Get tracks for enrichment const enrichmentTracks = await this.getTracksForEnrichment(20); logDebug('\n🎯 Priority Tracks for AI Enrichment:`); logDebug(\`─\`.repeat(60)); logDebug(`(Prioritized by country diversity and era)\n`);

        enrichmentTracks.slice(0, 10).forEach((track, i) => { const country = this.countries[track.country_code]; const year = track.year_code > 50 ? \`19${track.year_code}\` : `20${track.year_code}`; logDebug(\`${i + 1}. \`${track.title}` by ${track.artist}`); logDebug( \`   ${country?.emoji || \`🌍`} ${country?.name || track.country_code} (${year})`); logDebug(\`   Genre: ${track.genre || \`Unknown`}`);
            if (track.AI_ENERGY) { logDebug( `   Energy: ${(track.AI_ENERGY * 100).toFixed(0)}% | BPM: ${track.AI_BPM || 'N/A'}');
            }
            logDebug('');
        });
        ...
        logDebug('=`.repeat(60)); logInfo(`✅ Full report saved to: geographic-analysis-report.json\`);
