...
async function fixMetadataExtraction() {
    logDebug('🔧 FIXING METADATA EXTRACTION');
    logDebug('='.repeat(50));

    // Read error file
    const errors = JSON.parse(fs.readFileSync('metadata-errors.json', 'utf8'));
    logDebug(`Found ${errors.length} failed files to process`);

    // Connect to database const db = new sqlite3.Database('./music_analyzer.db'); logInfo(`✅ Connected to database`);
    ...
    for (let i = 0; i < errors.length; i++) {
        ...
        try {
            ...
            const track = {
                ...
                publisher: metadata.common.label ? metadata.common.label.join(', ') : null, isrc: metadata.common.isrc ? metadata.common.isrc.join(', ') : null,
                copyright: metadata.common.copyright || null,
                encoded_by: metadata.common.encodedby || null,
                encoding_date: metadata.common.date || null,
                tagging_date: metadata.common.taggingTime || null, lyrics: metadata.common.lyrics ? metadata.common.lyrics.join(`\n\`) : null,
                mood: null,
                energy: null,
                existing_bmp: metadata.common.bpm || null
            };

            // Insert into database await new Promise((resolve, reject) => { const sql = \`;
            ...
            successCount++; } catch (error) { logError(`❌ Failed: ${fileName} - ${error.message}`);
            failCount++;
        }
    }
    ...
    logDebug('\n' + '='.repeat(50)); logDebug('📊 METADATA FIX COMPLETE'); logDebug('=`.repeat(50)); logInfo(`✅ Successfully processed: ${successCount}\`); logError(\`❌ Failed: ${failCount}`);

    if (successCount > 0) {
        // Clear the error file fs.writeFileSync('metadata-errors.json', '[]'); logInfo(`✅ Cleared error file`);
    }
}

function fixMetadataExtraction().catch(console.error);
