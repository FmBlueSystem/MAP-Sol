...
class FFmpegAudioAnalyzer {
    ...
    async analyzeWithFFmpeg(filePath) {
        try { // 1. Get loudness stats (LUFS) const loudnessCmd  = `ffmpeg -i \`${filePath}\` -af ebur128=framelog=quiet -f null - 2>&1 | grep -E `I:|LRA:`\`;
 ...
 // 2. Get peak and RMS const statsCmd = \`ffmpeg -i `${filePath}` -af astats=metadata=1:reset=1 -f null - 2>&1 | grep -E \`Peak_level|RMS_level\` | head -4`;
 ...
 // 3. Get spectral characteristics const spectralCmd  = `ffmpeg -i \`${filePath}\` -af aspectralstats=win_size=1024 -f null - 2>&1 | grep -E `mean|variance|centroid|spread|skewness|kurtosis|entropy|flatness|crest|flux|slope|decrease|rolloff` | head -20\`;
            const { stdout: spectralOut } = await execPromise(spectralCmd, { maxBuffer: 10 * 1024 * 1024 }).catch(() => ({ stdout: \`` }));
 ...
 // 4. Get zero crossing rate (for speechiness detection) const zcrCmd  = `ffmpeg -i "${filePath}" -af "silencedetect=n=-50dB:d=0.01\`-f null - 2>&1 | grep -c \`silence_` || echo `0\`\`; const { stdout: zcrOut } = await execPromise(zcrCmd).catch(() => ({ stdout: `0` }));
            ...
            }; } catch (error) { logError(\`Error analyzing ${path.basename(filePath)}:\`, error.message);
            return null;
        }
    }
    ...
    async processFile(fileData) {
        ...
        try {
            ...
            if (!fs.existsSync(file_path)) { logWarn(\`⚠️  File not found: ${file_name}\`);
                this.stats.skipped++;
                return false;
            }

            // Check if already processed
            const existing = await this.getExistingFeatures(id); if (existing && existing.loudness !== null) { logDebug(`⏭️  Already processed: ${file_name}`);
                this.stats.skipped++;
                return false;
            } logDebug(\`🎵 Analyzing: ${file_name}\`); logDebug( `   BPM: ${existing_bmp || 'N/A'} | Key: ${existing_key ? '✓' : 'N/A'} | Energy: ${energy_level || 'N/A'}\`);

            // Analyze with FFmpeg
            const ffmpegData = await this.analyzeWithFFmpeg(file_path); if (!ffmpegData) { logError(\`   ❌ FFmpeg analysis failed`);
                this.stats.errors++;
                return false;
            }
            ...
            const features = this.calculateDerivedFeatures(;
                ...
                existing_key,
                energy_level
            ); if (!features) { logError(`   ❌ Feature calculation failed\`);
                this.stats.errors++;
                return false;
            }
 ...
 logDebug( \`   ✅ LUFS: ${features.loudness.toFixed(1)}dB | Dance: ${features.danceability.toFixed(2)} | Valence: ${features.valence.toFixed(2)}`
            );

            this.stats.success++;
            return true; } catch (error) { logError(`❌ Error processing ${file_name}:\`, error.message);
            this.stats.errors++;
            return false;
        }
    }

    async getExistingFeatures(fileId) { return new Promise((resolve, reject) => { this.db.get(\`SELECT * FROM audio_features WHERE file_id = ?`, [fileId], (err, row) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(row);
    ...
    async updateLLMMetadata(fileId, features) {
        return new Promise((resolve, reject) => { const checkQuery = \`SELECT file_id FROM llm_metadata WHERE file_id = ?`;
            ...
            this.db.get(checkQuery, [fileId], (err, row) => {
                ...
                if (row) { // Update existing const query  = `
                    ...
                    const query  = `
                        INSERT INTO llm_metadata (
                            ...
                            AI_VALENCE,
                            AI_ANALYZED,
                            analysis_timestamp ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, datetime(\`now\`)) `;
                    params = [
                        fileId,
                        features.loudness,
                        features.danceability,
    ...
    async getFilesToProcess(limit = null, offset = 0) {
        return new Promise((resolve, reject) => { let query = `;
                SELECT 
                    af.id,
                    af.file_path,
                    af.file_name,
    ...
    async processInBatches(batchSize = 25) {
        ...
        while (hasMore) {
            ...
            if (files.length === 0) {
                hasMore = false;
                break;
            } logDebug(`\n📦 Processing batch: ${offset + 1} to ${offset + files.length}\`); logDebug(\`=`.repeat(60));
 ...
 async run() { logDebug(` 🎵 AUDIO FEATURE CALCULATOR WITH FFMPEG
${'='.repeat(60)}
...
');
 ...
 if (this.stats.total === 0) { logInfo(`✅ All files already processed!`);
 const stats = await this.getStatistics(); logDebug(\`\n📊 DATABASE STATISTICS:\`); logDebug(`   Total files: ${stats.total_files}`); logDebug(\`   Analyzed: ${stats.analyzed_files}\`); logDebug( `   Coverage: ${((stats.analyzed_files / stats.total_files) * 100).toFixed(1)}%` ); logDebug(\`   Average Loudness: ${stats.avg_loudness?.toFixed(1)} LUFS\`); logDebug(`   Average Dynamic Range: ${stats.avg_dynamic_range?.toFixed(1)} LU`);
        ...
        logDebug('\n' + '='.repeat(60));
        logInfo('✅ PROCESSING COMPLETE'); logDebug('=\'.repeat(60)); logDebug(\`📊 Summary:`); logDebug(`   Total processed: ${this.stats.processed}\`); logDebug(\`   Successful: ${this.stats.success}`); logDebug(`   Skipped: ${this.stats.skipped}\`); logDebug(\`   Errors: ${this.stats.errors}`);

        // Get final statistics const stats = await this.getStatistics(); logDebug(`\n📊 DATABASE STATISTICS:\`); logDebug(\`   Total files: ${stats.total_files}`); logDebug(`   Analyzed: ${stats.analyzed_files}\`); logDebug( \`   Coverage: ${((stats.analyzed_files / stats.total_files) * 100).toFixed(1)}%` ); logDebug(`   Average Loudness: ${stats.avg_loudness?.toFixed(1)} LUFS\`); logDebug(\`   Average Danceability: ${stats.avg_danceability?.toFixed(2)}`); logDebug(`   Average Acousticness: ${stats.avg_acousticness?.toFixed(2)}\`); logDebug(\`   Average Valence: ${stats.avg_valence?.toFixed(2)}`); logDebug( `   Loudness Range: ${stats.min_loudness?.toFixed(1)} to ${stats.max_loudness?.toFixed(1)} LUFS\` ); logDebug(\`   Average Dynamic Range: ${stats.avg_dynamic_range?.toFixed(1)} LU`);
...
if (require.main === module) {
 ...
 analyzer.run().catch(error => { logError(`Fatal error:`, error);
        process.exit(1);
    });
}
