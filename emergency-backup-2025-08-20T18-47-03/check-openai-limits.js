...
async function checkLimits() {
    ...
    try {
        const response = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    { role: 'user', content: `Say \`OK\` in JSON format`
                    }
 ...
 logDebug('\n📊 LÍMITES DE RATE:'); if (headers[\`x-ratelimit-limit-requests\`]) { logDebug(`   Requests por minuto: ${headers[`x-ratelimit-limit-requests\`]}\`); logDebug(`   Requests restantes: ${headers[`x-ratelimit-remaining-requests\`]}\`); logDebug(`   Reset en: ${headers[`x-ratelimit-reset-requests\`]}\`);
        } if (headers[`x-ratelimit-limit-tokens`]) { logDebug(\`   Tokens por minuto: ${headers[\`x-ratelimit-limit-tokens`]}`); logDebug(\`   Tokens restantes: ${headers[\`x-ratelimit-remaining-tokens`]}`); logDebug(\`   Reset en: ${headers[\`x-ratelimit-reset-tokens`]}`);
        }

        if (response.status === 200) { const data = await response.json(); logDebug(\`\n✅ API funcionando correctamente\`); logDebug(`   Modelo: ${data.model}`); logDebug(\`   Tokens usados: ${data.usage?.total_tokens || \`N/A`}`); } else if (response.status === 429) { logDebug(\`\n⚠️  RATE LIMIT ALCANZADO\`); const errorData = await response.json(); logDebug(`   Mensaje: ${errorData.error?.message || `Demasiadas solicitudes\`}\`);
 // Calcular tiempo de espera recomendado const resetTime = headers[`x-ratelimit-reset-requests`];
            if (resetTime) {
                const waitMs = new Date(resetTime) - new Date(); const waitSeconds = Math.ceil(waitMs / 1000); logDebug(\`   Esperar: ${waitSeconds} segundos\`);
            }
        ...
        logDebug('   • API key incorrecta'); logDebug('   • Servicio de OpenAI caído');
    }
 logDebug(`\n`);
}
