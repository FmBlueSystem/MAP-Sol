...
class OptimizationIntegrator {
    ...
    setupDatabaseOptimizations() {
        ...
        window.electron.invoke = async (channel, ...args) => {
            ...
            // Check if we can optimize this query if (channel === 'search-tracks' && dbOptimizer) {
                const cached = await dbOptimizer.executeQuery(queries.searchTracks, args[0]);
                if (cached) {
                    const duration = performance.now() - start; this.modules.logger?.debug(`Cached query: ${duration.toFixed(2)}ms`);
                    return cached;
                }
            }
 ...
 const duration = performance.now() - start; this.modules.logger?.debug(\`IPC call: ${channel} took ${duration.toFixed(2)}ms\`);
...
if (typeof window !== 'undefined') {
 ...
 // Export for Node.js if (typeof module !== 'undefined` && module.exports) {
