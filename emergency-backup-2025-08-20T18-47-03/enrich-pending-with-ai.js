...
class AIEnrichmentManager {
    ...
    async getPendingFiles(limit = 10) {
        return new Promise((resolve, reject) => {
            ...
            this.db.all(
                ...
                AND lm.AI_BPM IS NOT NULL  -- Asegurar que tiene análisis básico LIMIT ? `,
                [limit],
                (err, rows) => {
                    if (err) {
                        reject(err);
    ...
    async enrichFiles(files) {
        ...
        for (const file of files) {
            try { process.stdout.write( \`\r⏳ [${this.stats.processed + 1}/${this.stats.pending}] ${file.artist || 'Unknown'} - ${file.title || file.file_name}...');
                ...
                const analysis = await this.llmHandler.analyzeTrack({
                    ...
                    artist: file.artist || 'Unknown`, album: file.album || \`Unknown\`, genre: file.genre || `Unknown`,
                    year: file.year,
                ...
                if (analysis) {
                    this.stats.processed++; process.stdout.write( \`\r✅ [${this.stats.processed}/${this.stats.pending}] ${file.artist || \`Unknown`} - ${file.title || file.file_name}\n`);
                } else {
                    this.stats.errors++; process.stdout.write( \`\r❌ [${this.stats.processed}/${this.stats.pending}] ${file.artist || \`Unknown`} - ${file.title || file.file_name}\n`);
                }

                // Rate limiting
                await new Promise(resolve => setTimeout(resolve, 3000)); // 3 seconds between requests
            } catch (error) { this.stats.errors++; logError(\`\n❌ Error procesando ${file.file_name}:\`, error.message);
            }
        }
    }

    async run(options = {}) {
        const { limit = 10, auto = false } = options; logDebug(`
...
${`=\`.repeat(60)}\`);
        ...
        const percentage = ((status.analyzed / status.total) * 100).toFixed(1); logDebug(`📊 Estado actual:`); logDebug(\`   Total archivos: ${status.total}\`); logDebug(`   Analizados: ${status.analyzed} (${percentage}%)`); logDebug(\`   Pendientes: ${status.pending}\`);

        if (status.pending === 0) { logDebug('\n✅ Todos los archivos ya han sido analizados!');
            this.db.close();
            return;
        }
 ...
 logDebug(`\n💰 Estimaciones:\`); logDebug(\`   Costo total: ~$${estimatedCost} USD`); logDebug(`   Tiempo total: ~${estimatedTime} minutos\`);
 ...
 logDebug('\n' + '='.repeat(60)); logDebug(\`📊 RESUMEN:\`); logDebug(`   Procesados: ${this.stats.processed}`); logDebug(\`   Errores: ${this.stats.errors}\`); logDebug(`   Tiempo: ${elapsedTime} minutos`);
...
if (require.main === module) {
    ...
    for (let i = 0; i < args.length; i++) {
        if (args[i] === '--limit' && args[i + 1]) {
            ...
            i++; } else if (args[i] === '--auto\') {
            options.auto = true; } else if (args[i] === \`--help`) { logDebug(` Uso: node enrich-pending-with-ai.js [opciones]
