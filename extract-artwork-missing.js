...
async function extractArtworkMissing() {
    console.log('🎨 EXTRACTOR DE CARÁTULAS FALTANTES');
    console.log('=====================================\n');
    ...
    db.all(sqlSimple, [], async (err, rows) => {
        ...
        console.log(`📊 Total de archivos en BD: ${rows.length}`);
        ...
        console.log(`🔍 Archivos sin carátula: ${missingArtwork.length}\n`);
        
        if (missingArtwork.length === 0) {
            console.log('✅ Todas las carátulas ya están extraídas!');
            db.close();
            return;
        }
            ...
            await Promise.all(batch.map(async (file) => {
                ...
                try {
                    ...
                    if (metadata.common.picture && metadata.common.picture.length > 0) {
                        ...
                        extracted++;
                        process.stdout.write(`✅ [${++processed}/${missingArtwork.length}] ${file.file_name}\r`);
                    } else {
                        failed++;
                        process.stdout.write(`⚠️ [${++processed}/${missingArtwork.length}] Sin carátula: ${file.file_name}\r`);
                    }
                } catch (error) {
                    failed++;
                    process.stdout.write(`❌ [${++processed}/${missingArtwork.length}] Error: ${file.file_name}\r`);
        ...
        console.log('\n\n📊 RESUMEN FINAL:');
        console.log('=================');
        console.log(`✅ Carátulas extraídas: ${extracted}`);
        console.log(`⚠️ Sin carátula embebida: ${failed - (missingArtwork.length - extracted - failed)}`);
        console.log(`❌ Archivos con error: ${missingArtwork.length - extracted - failed}`);
        console.log(`⏱️ Tiempo total: ${elapsed} segundos`);
        console.log(`🚀 Velocidad: ${(missingArtwork.length / elapsed).toFixed(1)} archivos/segundo`);
