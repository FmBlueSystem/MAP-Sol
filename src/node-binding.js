            ...
            this.db.all(query, [limit], async (err, rows) => {
                ...
                for (const row of rows) {
                    if (!fs.existsSync(row.file_path)) {
                        console.log(`⚠️ File not found: ${row.file_path}`);
                        continue;
                    }
                    ...
                    if (features) {
                        ...
                        await this.updateLLMMetadata(row.id, features);
                        
                        console.log(`✅ Saved: BPM=${features.bpm}, ` +
...
if (require.main === module) {
    ...
    switch(command) {
        case 'analyze':
            ...
            if (!filePath) {
                console.log('Usage: node node-binding.js analyze <file>');
                process.exit(1);
            }
        ...
        case 'batch':
            ...
            analyzer.processPendingFiles(limit).then(count => {
                console.log(`\n✅ Processed ${count} files`);
                return analyzer.getStatistics();
            }).then(stats => {
                console.log('\n📊 Database Statistics:');
                console.log(`   Total files: ${stats.total_files}`);
                console.log(`   Analyzed: ${stats.analyzed_files}`);
                console.log(`   Average Energy: ${stats.avg_energy?.toFixed(2) || 'N/A'}');
                console.log(`   Average Danceability: ${stats.avg_danceability?.toFixed(2) || 'N/A'}');
                console.log(`   Average Valence: ${stats.avg_valence?.toFixed(2) || 'N/A'}');
                console.log(`   Average Acousticness: ${stats.avg_acousticness?.toFixed(2) || 'N/A'}');
                analyzer.close();
            });
            break;
            
        case 'stats':
            analyzer.getStatistics().then(stats => {
                console.log('📊 Database Statistics:');
                console.log(`   Total files: ${stats.total_files}`);
                console.log(`   Analyzed: ${stats.analyzed_files}`);
                console.log(`   Pending: ${stats.total_files - stats.analyzed_files}`);
                console.log(`   Average Energy: ${stats.avg_energy?.toFixed(2) || 'N/A'}');
                console.log(`   Average Danceability: ${stats.avg_danceability?.toFixed(2) || 'N/A'}');
                console.log(`   Average Valence: ${stats.avg_valence?.toFixed(2) || 'N/A'}');
                analyzer.close();
