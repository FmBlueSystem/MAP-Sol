                ...
                results.forEach((data) => {
                    ...
                    stmt.run(params, (err) => {
                        if (err) {
                            ...
                            db.run('UPDATE audio_files SET normalization_analyzed = 1 WHERE id = ?', [data.track_id]);
                        }
                    });
                });
                ...
                Object.entries(queries).forEach(([key, sql]) => {
                    db.get(sql, (err, row) => {
                        ...
                        if (completed === Object.keys(queries).length) {
                            stats.percentage = stats.total > 0 ? ((stats.analyzed / stats.total) * 100).toFixed(1) : 0;
                            resolve(stats);
                        }
                    });
                });
