...
currentDb.all('PRAGMA table_info(llm_metadata)', (err, currentColumns) => {
    ...
    if (fs.existsSync(backupPath)) {
        ...
        backupDb.all('PRAGMA table_info(llm_metadata)', (err, backupColumns) => {
            ...
            logDebug(
                `   Reducción: ${backupColumns.length - currentColumns.length} columnas eliminadas`
            );
            logDebug(
                `   Porcentaje: ${((1 - currentColumns.length / backupColumns.length) * 100).toFixed(1)}% menos`
            );

            // Encontrar columnas eliminadas
            const currentNames = new Set(currentColumns.map(c => c.name));
            const removedColumns = backupColumns
                .map(c => c.name)
