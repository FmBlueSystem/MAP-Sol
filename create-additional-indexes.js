...
console.log('🔧 CREANDO ÍNDICES SQL ADICIONALES');
console.log('=====================================\n');
...
function createIndex(indexName, tableName, columns, callback) {
    ...
    db.run(sql, (err) => {
        if (err) {
            console.log(`❌ Error creando ${indexName}: ${err.message}`);
        } else {
            console.log(`✅ Índice ${indexName} creado/verificado`);
        }
        callback();
    });
}
...
function createNextIndex() {
    if (currentIndex >= indexes.length) {
        ...
        db.run('VACUUM', (err) => {
            if (err) {
                console.log('❌ Error optimizando:', err.message);
            } else {
            ...
            db.run('ANALYZE', (err) => {
                if (err) {
                    console.log('❌ Error analizando:', err.message);
                } else {
                ...
                db.get("SELECT COUNT(*) as count FROM audio_files", (err, row) => {
                    ...
                    console.log(`💾 Tamaño de BD: ${fileSizeInMB.toFixed(2)} MB`);
