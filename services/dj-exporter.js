...
class DJExporter {
    ...
    exportRekordbox(tracks, collectionName = 'Collection') {
        const xml = xmlbuilder.create('DJ_PLAYLISTS', { version: '1.0.0', encoding: 'UTF-8' })
            .att('Version', '1.0.0');
        ...
        const collection = xml.ele('COLLECTION')
        ...
        const root = playlists.ele('NODE')
            .att('Type', '0')
            .att('Name', 'ROOT')
            .att('Count', '1');
        
        const playlist = root.ele('NODE')
    ...
    exportTraktor(tracks, collectionName = 'Collection') {
        const xml = xmlbuilder.create('NML', { version: '19', encoding: 'UTF-8' })
            .att('VERSION', '19');
        
        const head = xml.ele('HEAD')
        ...
        // Playlists
        const playlists = xml.ele('PLAYLISTS');
        const node = playlists.ele('NODE')
            .att('TYPE', 'FOLDER')
            .att('NAME', '$ROOT');
        
        const playlist = node.ele('SUBNODES').ele('NODE')
            .att('TYPE', 'PLAYLIST')
            .att('NAME', collectionName);
        
        const playlistEntries = playlist.ele('PLAYLIST')
            .att('ENTRIES', tracks.length)
            .att('TYPE', 'LIST')
            .att('UUID', this.generateUUID());
        
        tracks.forEach((track, index) => {
            playlistEntries.ele('ENTRY').ele('PRIMARYKEY')
                .att('TYPE', 'TRACK')
