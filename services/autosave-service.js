...
class AutoSaveService {
    constructor(storageDir = null) {
        this.storageDir = storageDir || path.join(process.env.HOME || process.env.USERPROFILE, '.music-analyzer', 'recovery');
    ...
    start(sessionData = {}) {
        ...
        console.log(`✅ Auto-save iniciado (cada ${this.saveInterval/1000}s)`);
        ...
        if (files.length > this.maxRecoveryFiles) {
            files.slice(this.maxRecoveryFiles).forEach(f => {
                try {
                    ...
                    console.log(`🗑️ Eliminado archivo de recuperación antiguo: ${f.file}`);
                } catch (error) {
                    console.error('Error deleting old recovery file:', error);
                }
            });
        }
    }
    ...
    clearAll() {
        ...
        files.forEach(f => {
            try {
                ...
                console.error('Error deleting file:', f, error);
            }
        });
        
        console.log(`🗑️ Eliminados ${deleted} archivos de recuperación`);
        return deleted;
    }
}
