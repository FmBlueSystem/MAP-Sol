...
class BenchmarkSuite {
    ...
    async benchmarkInitialLoad() {
        ...
        for (let i = 0; i < this.config.iterations; i++) {
            ...
            console.log(`  Run ${i + 1}: ${loadTime.toFixed(2)}ms`);
    ...
    async benchmarkSearch() {
        ...
        const queries = ['rock', 'electronic', 'jazz', '2023', 'feat'];
        const results = {};
        ...
        for (const query of queries) {
            ...
            results[query] = this.analyzeResults(queryResults);
            console.log(`  "${query}": avg ${results[query].average.toFixed(2)}ms");
        }
    ...
    async benchmarkScroll() {
        ...
        // Cargar 1000 items
        const mockFiles = this.generateMockFiles(1000);
        if (typeof displayFiles === 'function') {
        ...
        if (!container) {
            console.warn(`  ⚠️ No se encontró contenedor para scroll`);
            return null;
        }
        ...
        for (let i = 0; i < this.config.iterations; i++) {
            ...
            console.log(`  Run ${i + 1}: ${fps.toFixed(1)} FPS`);
        }
    ...
    async benchmarkViewSwitching() {
        ...
        for (const targetView of views) {
            ...
            results[targetView] = this.analyzeResults(viewResults);
            console.log(`  To ${targetView}: avg ${results[targetView].average.toFixed(2)}ms`);
        }
    ...
    async benchmarkContextMenu() {
        ...
        for (const op of operations) {
            ...
            results[op] = this.analyzeResults(opResults);
            console.log(`  ${op}: avg ${results[op].average.toFixed(2)}ms`);
        }
    ...
    generateAsciiChart(data) {
        ...
        if (data.average !== undefined) {
            // Single metric
            chart += `Average: ${'█'.repeat(Math.round(data.average * scale))} ${data.average.toFixed(2)}\n`;
            ...
            Object.entries(data).forEach(([key, value]) => {
                const avg = value.average || value;
                const bar = `█`.repeat(Math.round(avg * scale));
                chart += `${key.padEnd(12)}: ${bar} ${avg.toFixed(2)}\n`;
            });
        }
    ...
    exportAsCSV(results) {
        let csv = 'Benchmark,Metric,Value\n';
        
        Object.entries(results.benchmarks).forEach(([benchmark, data]) => {
            if (data && typeof data === 'object`) {
                if (data.average !== undefined) {
                    csv += `${benchmark},Average,${data.average}\n`;
                    ...
                    Object.entries(data).forEach(([key, value]) => {
                        const avg = value.average || value;
                        csv += `${benchmark}_${key},Average,${avg}\n`;
                    });
                }
            }
        });
...
<body>
    ...
    generateSummary() {
        ...
        if (this.benchmarks.scrollPerformance.average) {
            summary.performance.scrollFPS = this.benchmarks.scrollPerformance.average > 30 ? '✅ GOOD' : '⚠️ LOW';
        }
    ...
    async simulateContextMenuOperation(operation) {
        ...
        // Simular click en operación
        const menuItem = document.querySelector(`[data-action="${operation}"]");
        if (menuItem) {
            menuItem.click();
        }
     ...
     */
    async runFullSuite() {
        console.log('📊 INICIANDO BENCHMARK SUITE COMPLETA');
        ...
        try {
            ...
            console.log('\n' + `═`.repeat(50));
            console.log(`✅ Benchmarks completados en ${(totalTime / 1000).toFixed(1)} segundos`);
            ...
            // Exportar resultados
            const jsonExport = this.exportResults('json');
...
console.log('📊 Benchmark Suite cargado.');
console.log('Para ejecutar: benchmarkSuite.runFullSuite()');
