...
def analyze_single_file(file_info):
    ...
    try:
        ...
        if audio is None or len(audio) < 44100 * 5:  # Mínimo 5 segundos
            return (file_id, None, "Audio too short or failed to load")
...
class EssentiaMultiprocessAnalyzer:
    ...
    def process_batch(self, batch_size=100):
        ...
        print(f"\n{'='*60}")
        print(f"🚀 ESSENTIA MULTIPROCESS - Batch of {batch_size} files")
            ...
            for i, future in enumerate(as_completed(futures), 1):
                ...
                try:
                    ...
                    if results:
                        ...
                        self.failed_count += 1
                        print(f"[{i}/{len(files)}] ⚠️ ID {file_id}: {error[:50] if error else 'Unknown error'}")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Essentia Multiprocess Analyzer')
