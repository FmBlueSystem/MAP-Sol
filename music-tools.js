...
function showHelp() {
    logDebug(`
🎵 MUSIC ANALYZER PRO - HERRAMIENTAS
=====================================
...
switch (command) {
    ...
    case 'stats':
        // Crear script de estadísticas si no existe
        const statsScript = `
...
\', (err, files) => {
    ...
    \`, (err, metadata) => {
        ...
        logDebug(\`   Con carátula: \${files.with_artwork} (\${(files.with_artwork/files.total*100).toFixed(1)}%)\`);
        logDebug(\`   Con análisis IA: \${metadata.with_ai} (\${(metadata.with_ai/files.total*100).toFixed(1)}%)\`);
        ...
        \`, (err, eras) => {
            ...
            \`, (err, genres) => {
                ...
                genres.forEach((genre, i) => {
                    logDebug(\`   \${(i+1).toString().padStart(2)}. \${genre.genre}: \${genre.count} tracks\`);
                });
                
                // Resumen
                logDebug('\\n✨ RESUMEN:');
                logDebug(\`   Pendientes de análisis IA: \${files.total - metadata.with_ai}\`);
                logDebug(\`   Costo estimado para completar: ~$\${((files.total - metadata.with_ai) * 0.01).toFixed(2)} USD\`);
                logDebug(\`   Tiempo estimado: ~\${Math.ceil((files.total - metadata.with_ai) / 100)} horas\`);
        ...
        const fs = require('fs');
        const tmpScript = path.join(__dirname, 'show-database-stats.js');
        fs.writeFileSync(tmpScript, statsScript);
        runCommand('show-database-stats.js');
        break;

    default:
        logError(`❌ Comando desconocido: ${command}`);
        logDebug('Usa "node music-tools.js help" para ver los comandos disponibles');
        process.exit(1);
}
