...
class MusicAnalyzerUI {
            ...
            <div id="analyzerModal" class="analyzer-modal" style="display: none;">
                <div class="analyzer-modal-content">
                    ...
                    <div class="analyzer-footer">
                        ...
                        <button class="btn-warning" id="pauseBtn" style="display: none;">Pause</button>
                        <button class="btn-success" id="resumeBtn" style="display: none;">Resume</button>
                        <button class="btn-primary" id="doneBtn" style="display: none;">Done</button>
                    </div>
                </div>
            </div>
        ";
    ...
    async selectFolder() {
        ...
        if (window.electronAPI && window.electronAPI.invoke) {
            ...
            alert('File selection not available. Please restart the application.');
        }
    }
        ...
        if (window.electronAPI && window.electronAPI.invoke) {
            try {
                ...
                if (result.success && result.paths) {
                    this.selectedFiles = result.paths;
                    this.showSelectedInfo(`${result.paths.length} files`, 'files');
                }
            } catch (error) {
    ...
    async analyzeAll() {
        ...
        if (window.electronAPI && window.electronAPI.invoke) {
            try {
                const result = await window.electronAPI.invoke('get-all-library-files');
                if (result.success && result.paths) {
                    this.selectedFiles = result.paths;
                    this.showSelectedInfo(
                        `Entire Library (${result.paths.length} files)`,
                        'library'
                    );
                } else {
    ...
    showSelectedInfo(selection, type) {
        ...
        if (type === 'folder') {
            ...
            // Show file count
            const count = parseInt(selection.split(' ')[0]) || this.selectedFiles.length;
            document.getElementById('selectedPath').textContent = `${count} audio files selected`;
        ...
        if (window.electronAPI && window.electronAPI.invoke) {
            ...
            try {
                ...
                if (this.selectedFiles.length === 1 && !this.selectedFiles[0].includes('.')) {
                    ...
                    // Individual files
                    result = await window.electronAPI.invoke('analyze-files', this.selectedFiles);
                }
            ...
            alert('ElectronAPI not available. Please restart the application.');
        }
    }
    ...
    updateProgress(progress) {
        ...
        const step = Math.ceil((progress.percentage / 100) * 4);
        for (let i = 1; i <= 4; i++) {
        ...
        // Add to log
        this.addToLog(`Processing: ${progress.currentFile || 'Unknown'}');
    }
    ...
    showResults(result) {
        ...
        if (result && result.result) {
            ...
            document.getElementById('successCount').textContent = data.processed - data.failed || 0;
            document.getElementById('failedCount').textContent = data.failed || 0;
            document.getElementById('totalTime').textContent = `${Math.round(data.duration || 0)}s`;
