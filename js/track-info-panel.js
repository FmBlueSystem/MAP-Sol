...
class TrackInfoPanel {
    ...
    getGradientColor(value) {
        ...
        if (value < 0.3) return '#3498db'; // Azul
        if (value < 0.6) return '#2ecc71'; // Verde
        if (value < 0.8) return '#f39c12'; // Naranja
        return '#e74c3c'; // Rojo
    }
    ...
    updateTags(track) {
        ...
        if (track.AI_ENERGY > 0.7) tags.push('High Energy');
        if (track.AI_DANCEABILITY > 0.7) tags.push('Danceable');
        if (track.AI_VALENCE > 0.7) tags.push('Happy');
        if (track.AI_VALENCE < 0.3) tags.push('Dark');
        if (track.AI_ACOUSTICNESS > 0.7) tags.push('Acoustic');
        if (track.AI_INSTRUMENTALNESS > 0.7) tags.push('Instrumental');
        if (track.AI_LIVENESS > 0.7) tags.push('Live');
        if (track.AI_SPEECHINESS > 0.3) tags.push('Vocal');
        
        // Agregar mood y genre como tags
        if (track.AI_MOOD) tags.push(track.AI_MOOD);
    ...
    findSimilar() {
        ...
        document.dispatchEvent(new CustomEvent('findSimilarTracks', {
    ...
    analyzeDeep() {
        ...
        document.dispatchEvent(new CustomEvent('deepAnalysis', {
            detail: this.currentTrack
        }));
    }
