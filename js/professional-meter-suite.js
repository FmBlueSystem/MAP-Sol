...
class ProfessionalMeterSuite {
    ...
    connectToAudio(audioElement) {
        ...
        try {
            ...
            if (audioElement._sourceNode) {
                ...
                console.log('📊 Created new meter source');
            }
            ...
            try {
                ...
                console.log('📊 Audio already routed to destination');
            }
    ...
    updateLUFSValues() {
        ...
        if (this.measurementHistory.length > 0) {
            ...
            if (gatedMeasurements.length > 0) {
                const sum = gatedMeasurements.reduce((acc, m) => acc + Math.pow(10, m.lufs / 10), 0);
    ...
    updateTruePeakMeter(channel, value, holdValue) {
        ...
        if (bar && valueEl) {
            ...
            if (value > 0) {
                ...
                valueEl.style.color = '#ffff00';
                bar.style.background = 'linear-gradient(90deg, #00ff00 0%, #ffff00 80%, #ff0000 100%)';
            } else {
                valueEl.style.color = '#00ff00';
                bar.style.background = 'linear-gradient(90deg, #00ff00 0%, #00ff00 70%, #ffff00 100%)';
            }
        }
    }
    ...
    updateStatistics() {
        ...
        if (this.elements.statMaxTP) {
            this.elements.statMaxTP.textContent = this.maxTruePeak > -60 ? 
                this.maxTruePeak.toFixed(1) + ' dBTP' : '-∞ dBTP';
        }
    ...
    setStandard(standard) {
        ...
        console.log(`📊 Target LUFS set to ${this.config.targetLUFS}`);
    }
    ...
    toggleFreeze() {
        ...
        console.log(`📊 Meter ${this.isFrozen ? 'frozen' : 'unfrozen'}`);
    }
    ...
    disconnect() {
        ...
        // Clean up filters
        if (this.preFilterL) this.preFilterL.disconnect();
        if (this.preFilterR) this.preFilterR.disconnect();
        if (this.rlbFilterL) this.rlbFilterL.disconnect();
        if (this.rlbFilterR) this.rlbFilterR.disconnect();
        if (this.analyserL) this.analyserL.disconnect();
        if (this.analyserR) this.analyserR.disconnect();
        if (this.analyserMono) this.analyserMono.disconnect();
        if (this.splitter) this.splitter.disconnect();
        
        this.isConnected = false;
        
        console.log('🔌 Professional Meter Suite disconnected');
    }
}
...
console.log('📊 Professional Meter Suite loaded - Broadcast Quality');