    ...
    setupEventListeners() {
        ...
        if (volumeSlider) {
            volumeSlider.addEventListener('input', (e) => {
                ...
                if (window.player && window.player.currentAudio) {
                    window.player.currentAudio.volume = volume;
                }
        ...
        if (progressContainer) {
            progressContainer.addEventListener('click', (e) => {
                if (window.player && window.player.currentAudio && window.player.currentAudio.duration) {
                    const rect = progressContainer.getBoundingClientRect();
                    const percent = (e.clientX - rect.left) / rect.width;
                    window.player.currentAudio.currentTime = percent * window.player.currentAudio.duration;
                }
            });
        }
    }
    ...
    startMonitoring() {
        ...
        setInterval(() => {
            if (window.player && player.currentAudio && !player.currentAudio.paused) {
                ...
                if (playBtn) {
                    playBtn.innerHTML =
                        '<svg width="32" height="32" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="6" width="4" height="12"/><rect x="14" y="6" width="4" height="12"/></svg>';
                }
            } else {
                const playBtn = document.getElementById('play-pause-btn');
                if (playBtn) {
...
function togglePlayPause() {
    if (window.player) {
        if (window.player.isPlaying) {
            window.player.pause();
        } else if (window.player.currentAudio) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.audioPanel.waveformCanvas) {
        window.audioPanel.drawWaveform();
    }
});
...
if (window.player) {
    ...
    const originalPlay = window.player.play.bind(window.player);
    window.player.play = function (trackPath, trackId, trackData) {
        ...
        if (window.audioPanel) {
            ...
            setTimeout(() => {
                if (window.player.currentAudio) {
                    window.audioPanel.setupAudioAnalysis(window.player.currentAudio);
                    window.audioPanel.updateTrackInfo(trackData || window.player.currentTrackData);
                }
            }, 100);
        }
    };
    ...
    const originalStop = window.player.stop ? window.player.stop.bind(window.player) : null;
