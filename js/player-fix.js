...
class FixedPlayerSystem {
    ...
    startVisualization() {
        ...
        const updateMeters = () => {
            ...
            // Actualizar meters
            this.updateMeter('l', dbL);
            this.updateMeter('r', dbR);
    ...
    updateMeter(channel, dbValue) {
        ...
        const meterEl = document.getElementById(`meter-${channel}`);
        if (meterEl) {
            ...
            if (db > 0) {
                ...
                meterEl.style.background = 'linear-gradient(90deg, #00ff00, #ffff00)';
            } else {
                meterEl.style.background = '#00ff00';
            }
        }

        // Actualizar valor numérico
        const dbEl = document.getElementById(`db-${channel}`);
        if (dbEl) {
...
window.playTrackFromCard = function (trackData, index) {
    if (!fixedPlayerSystem) {
        fixedPlayerSystem = new FixedPlayerSystem();
    }
